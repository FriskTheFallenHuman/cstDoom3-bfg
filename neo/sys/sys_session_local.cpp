...
//#modified-fva; BEGIN
idCVar cst_grenadesToggle("cst_grenadesToggle", "0", CVAR_SYSTEM | CVAR_BOOL | CVAR_ARCHIVE, "enable the grenades key to toggle between grenades and the previous weapon");
idCVar cst_autoReloadAlwaysDbs("cst_autoReloadAlwaysDbs", "0", CVAR_SYSTEM | CVAR_BOOL | CVAR_ARCHIVE, "always auto reload the Double Barrel Shotgun");
//#modified-fva; END
...
idCVar net_port("net_port", "27015", CVAR_INTEGER | CVAR_NOCHEAT, "port number");
...
struct netVersion_s {
	...
	netVersion_s() { sprintf(string, "%s.%d", CST_ENGINE_ID, CST_NET_VERSION); }
	//#modified-fva; END
	char	string[256];
} netVersion;
...
bool lobbyUser_t::UpdateClientMutableData( const idLocalUser * localUser ) {
	...
	//#modified-fva; BEGIN
	updated |= CheckAndUpdateValue(cstGrenadesToggle, cst_grenadesToggle.GetBool());
	updated |= CheckAndUpdateValue(cstAutoReloadAlwaysDbs, cst_autoReloadAlwaysDbs.GetBool());
	//#modified-fva; END
	return updated;
}
...
void idSessionLocal::HandleDedicatedServerQueryRequest( lobbyAddress_t & remoteAddr, idBitMsg & msg, int msgType ) {
	...
	if (masterUser == NULL && !common->CstGetServerDedicated()) {
	//#modified-fva; END
		canJoin = false;
	}
	...
	if ( canJoin ) {
		...
		//#modified-fva; BEGIN
		//if ( !net_headlessServer.GetBool() ) {
		if (!common->CstGetServerDedicated()) {
		//#modified-fva; END
