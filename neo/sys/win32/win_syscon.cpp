...
void Sys_ShowConsole( int visLevel, bool quitOnClose ) {
	...
	switch ( visLevel ) {
		...
		//#modified-fva; BEGIN
		case 3: {
			// same as case 1, but also centers the window
			ShowWindow(s_wcd.hWnd, SW_SHOWNORMAL);
			SendMessage(s_wcd.hwndBuffer, EM_LINESCROLL, 0, 0xffff);

			HDC hDC = GetDC(GetDesktopWindow());
			if (hDC) {
				int desktopWidth = GetDeviceCaps(hDC, HORZRES);
				int desktopHeight = GetDeviceCaps(hDC, VERTRES);
				ReleaseDC(GetDesktopWindow(), hDC);
				SetWindowPos(s_wcd.hWnd, HWND_NOTOPMOST, (desktopWidth - s_wcd.windowWidth) / 2, (desktopHeight - s_wcd.windowHeight) / 2, 0, 0, SWP_NOSIZE);
			}
			break;
		}
		//#modified-fva; END
