...
void idLobby::HandlePacket( lobbyAddress_t & remoteAddress, idBitMsg fragMsg, idPacketProcessor::sessionId_t sessionID ) {
	...
	if ( type == idPacketProcessor::RETURN_TYPE_OOB ) {
		if ( userData == OOB_VOICE_AUDIO ) {
			...
			//#modified-fva; BEGIN
			// see idLobbyBackendDirect::GetConnectInfo
...
bool idLobby::CheckVersion( idBitMsg & msg, lobbyAddress_t peerAddress ) {
	const unsigned long remoteChecksum = msg.ReadInt();
...
//#modified-fva; BEGIN
bool idLobby::CstGetLobbyUserGrenadesToggle(lobbyUserID_t lobbyUserID) const {
	const int userIndex = GetLobbyUserIndexByID(lobbyUserID);
	const lobbyUser_t *user = GetLobbyUser(userIndex);
	return user ? user->cstGrenadesToggle : false;
}

bool idLobby::CstGetLobbyUserAutoReloadAlwaysDbs(lobbyUserID_t lobbyUserID) const {
	const int userIndex = GetLobbyUserIndexByID(lobbyUserID);
	const lobbyUser_t *user = GetLobbyUser(userIndex);
	return user ? user->cstAutoReloadAlwaysDbs : false;
}
//#modified-fva; END
...
void idLobby::HandleReliableMsg(int p, idBitMsg & msg, const lobbyAddress_t & cstRemoteAddress) {
//#modified-fva; END
	peer_t & peer = peers[p];
	...
	if ( reliableType == RELIABLE_HELLO ) {
		...
		//#modified-fva; BEGIN
		// see idLobbyBackendDirect::GetConnectInfo
		connectInfo.netAddr = cstRemoteAddress.netAddr;
		//#modified-fva; END
