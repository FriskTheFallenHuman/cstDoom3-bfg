...
idRenderModel *R_EntityDefDynamicModel( idRenderEntityLocal *def ) {
	...
	//#modified-fva; BEGIN
	bool cstGpuSkinningUpdate = false;
	if (def->dynamicModel && def->dynamicModel->CstIsGpuSkinningSyncRequired()) {
		short dynModelGpuSkinning = def->dynamicModel->CstGetUseGpuSkinning();
		//if (dynModelGpuSkinning != -1 && dynModelGpuSkinning != (short)r_useGPUSkinning.GetBool()) {
		if (dynModelGpuSkinning != -1 && dynModelGpuSkinning != (short)gCstLatched__r_useGPUSkinning) {
			cstGpuSkinningUpdate = true;
		}
	}

	bool cstShowSkelUpdate = false;
	if ((def->dynamicModel && def->dynamicModel->CstGetShowingSkel()) || r_showSkel.GetBool()) {
		cstShowSkelUpdate = true;
	}
	//#modified-fva; END
	...
	if (callbackUpdate || (model->IsDynamicModel() == DM_CONTINUOUS && def->dynamicModelFrameCount != tr.frameCount) || cstGpuSkinningUpdate || cstShowSkelUpdate) {
	//#modified-fva; END
	...
	//#modified-fva; BEGIN
	if (def->dynamicModel && def->dynamicModel->CstIsGpuSkinningSyncRequired()) {
		//def->dynamicModel->CstSetUseGpuSkinning(r_useGPUSkinning.GetBool());
		def->dynamicModel->CstSetUseGpuSkinning(gCstLatched__r_useGPUSkinning);
	}
	//#modified-fva; END
...
void R_AddSingleModel( viewEntity_t * vEntity ) {
	...
	for ( int surfaceNum = 0; surfaceNum < model->NumSurfaces(); surfaceNum++ ) {
		...
		const bool surfaceDirectlyVisible = modelIsVisible && shader->IsDrawn() && !idRenderMatrix::CullBoundsToMVP(vEntity->mvp, tri->bounds);
		const bool gpuSkinned = (tri->staticModelWithJoints != NULL && gCstLatched__r_useGPUSkinning);
		//#modified-fva; END
		...
		for ( int contactedLight = 0; contactedLight < numContactedLights; contactedLight++ ) {
			...
			const bool forceShadowCaps = !addInteractions || !shader->IsDrawn() || r_forceShadowCaps.GetBool();
			//#modified-fva; END
...
void R_AddModels() {
	...
	extern bool gCstLatched__r_useParallelAddModels;
	if (gCstLatched__r_useParallelAddModels) {
	//#modified-fva; END
	...
	extern bool gCstLatched__r_useParallelAddShadows;
	if (gCstLatched__r_useParallelAddShadows) {
	//#modified-fva; END
		for ( viewEntity_t * vEntity = tr.viewDef->viewEntitys; vEntity != NULL; vEntity = vEntity->next ) {
