...
void idMD5Mesh::UpdateSurface( const struct renderEntity_s *ent, const idJointMat *entJoints,
	...
	if (gCstLatched__r_useGPUSkinning) {
	//#modified-fva; END
...
idRenderModel *idRenderModelMD5::InstantiateDynamicModel( const struct renderEntity_s *ent, const viewDef_t *view, idRenderModel *cachedModel ) {
	...
	//#modified-fva; BEGIN
	staticModel->CstSetRequiresGpuSkinningSync(false);
	staticModel->CstSetShowingSkel(r_showSkel.GetBool());
	//#modified-fva; END
	...
	//#modified-fva; BEGIN
	staticModel->CstSetRequiresGpuSkinningSync(true);
	//#modified-fva; END
