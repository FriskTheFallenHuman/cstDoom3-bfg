...
*/
static const int CST_DECALS_GRANULARITY = 64;
static const int CST_DEFERRED_DECALS_GRANULARITY = 8;
idRenderModelDecal::idRenderModelDecal() :
	cstDecals(CST_DECALS_GRANULARITY),
	cstDeferredDecals(CST_DEFERRED_DECALS_GRANULARITY),
	cstDecalMaterials(CST_DECALS_GRANULARITY) {
}
//#modified-fva; END
...
void idRenderModelDecal::CreateDecalFromWinding( const idWinding &w, const idMaterial *decalMaterial, const idPlane fadePlanes[2], float fadeDepth, int startTime ) {
	...
	*/
	int decalIndex = cstDecals.Num() - 1;
	if (decalIndex >= 0
		&& cstDecals[decalIndex].material == decalMaterial
		&& cstDecals[decalIndex].startTime == startTime
		&& cstDecals[decalIndex].numVerts + w.GetNumPoints() <= MAX_DECAL_VERTS
		&& cstDecals[decalIndex].numIndexes + 3 * (w.GetNumPoints() - 2) <= MAX_DECAL_INDEXES) {
	} else {
		decal_t &newDecal = cstDecals.Alloc();
	...
	decal_t &decal = cstDecals[cstDecals.Num() - 1];
	//#modified-fva; END
...
void idRenderModelDecal::CreateDeferredDecals( const idRenderModel *model ) {
	...
	nextDeferredDecal = 0;
	*/
	if (cstDeferredDecals.Num()) {
		for (int i = 0; i < cstDeferredDecals.Num(); ++i) {
			decalProjectionParms_t &parms = cstDeferredDecals[i];
			CreateDecal(model, parms);
		}
		cstDeferredDecals.Clear();
	}
	//#modified-fva; END
}
...
void idRenderModelDecal::AddDeferredDecal( const decalProjectionParms_t &localParms ) {
	...
	*/
	cstDeferredDecals.Append(localParms);
	//#modified-fva; END
}
...
*/
void idRenderModelDecal::RemoveFadedDecals(int time) {
	int oldNum = cstDecals.Num();
	int i = 0;
	while (i < cstDecals.Num()) {
		const decalInfo_t decalInfo = cstDecals[i].material->GetDecalInfo();
		const int minTime = time - (decalInfo.stayTime + decalInfo.fadeTime);
		if (cstDecals[i].startTime <= minTime) {
			cstDecals.RemoveIndexFast(i);
		} else {
			++i;
		}
	}
	if (oldNum != cstDecals.Num()) {
		cstDecals.SetGranularity(cstDecals.GetGranularity()); // downsize using the same granularity
	}

	oldNum = cstDeferredDecals.Num();
	i = 0;
	while (i < cstDeferredDecals.Num()) {
		const decalInfo_t decalInfo = cstDeferredDecals[i].material->GetDecalInfo();
		const int minTime = time - (decalInfo.stayTime + decalInfo.fadeTime);
		if (cstDeferredDecals[i].startTime <= minTime) {
			cstDeferredDecals.RemoveIndexFast(i);
		} else {
			++i;
		}
	}
	if (oldNum != cstDeferredDecals.Num()) {
		cstDeferredDecals.SetGranularity(cstDeferredDecals.GetGranularity()); // downsize using the same granularity
	}
}
//#modified-fva; END
...
*/
unsigned int idRenderModelDecal::GetNumDecalDrawSurfs() {
	cstDecalMaterials.Clear();
	for (int i = 0; i < cstDecals.Num(); ++i) {
		cstDecalMaterials.AddUnique(cstDecals[i].material);
	}
	return cstDecalMaterials.Num();
}
//#modified-fva; END
...
drawSurf_t * idRenderModelDecal::CreateDecalDrawSurf( const viewEntity_t *space, unsigned int index ) {
	...
	*/
	if ((int)index >= cstDecalMaterials.Num()) {
		return NULL;
	}

	const idMaterial *material = cstDecalMaterials[index];

	int maxVerts = 0;
	int maxIndexes = 0;
	for (int i = 0; i < cstDecals.Num(); ++i) {
		const decal_t &decal = cstDecals[i];
		if (decal.material == material) {
			maxVerts += decal.numVerts;
			maxIndexes += decal.numIndexes;
		}
