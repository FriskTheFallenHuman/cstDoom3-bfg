...
//#modified-fva; BEGIN
idCVar cst_intelWorkaroundsSyncType("cst_intelWorkaroundsSyncType", "0", CVAR_RENDERER | CVAR_INTEGER | CVAR_NOCHEAT, "sync strategy involving glFinish and SwapBuffers to use with Intel video cards when r_skipIntelWorkarounds is 0:\n  0 = finish, swap\n  1 = swap, finish\n  2 = swap, small draw, finish\n  3 = finish, swap, finish\n  4 = finish, swap, small draw, finish", 0, 4);
//#modified-fva; END
...
const void GL_BlockingSwapBuffers() {
	...
	*/
	if (!glConfig.syncAvailable) {
		int syncType = cst_intelWorkaroundsSyncType.GetInteger();
	...
	for ( int stereoEye = 1; stereoEye >= -1; stereoEye -= 2 ) {
		...
		for ( const emptyCommand_t * cmds = allCmds; cmds != NULL; cmds = (const emptyCommand_t *)cmds->next ) {
			...
			case RC_POST_PROCESS:
				...
				break;
			//#modified-fva; BEGIN
			case RC_CST_UPDATE_CINEMATIC:
				RB_CstUpdateCinematic(cmds);
				break;
			//#modified-fva; END
			default:
