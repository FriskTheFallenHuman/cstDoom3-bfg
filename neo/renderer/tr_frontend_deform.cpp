...
static drawSurf_t * R_AutospriteDeform( drawSurf_t *surf ) {
	...
	const idJointMat * joints = (srcTri->staticModelWithJoints != NULL && gCstLatched__r_useGPUSkinning) ? srcTri->staticModelWithJoints->jointsInverted : NULL;
	//#modified-fva; END
...
static void AddTriangleToIsland_r( const srfTriangles_t *tri, int triangleNum, bool *usedList, eyeIsland_t *island ) {
	...
	const idJointMat * joints = (tri->staticModelWithJoints != NULL && gCstLatched__r_useGPUSkinning) ? tri->staticModelWithJoints->jointsInverted : NULL;
	//#modified-fva; END
