...
//#modified-fva; BEGIN
void idRenderSystemLocal::CstSetShaderParms(int timeMsec, const idVec4 &colorRgba) {
	cstUseShaderParms = true;
	cstShaderMsec = timeMsec;
	cstShaderColor = colorRgba;
}
//#modified-fva; END
...
void idRenderSystemLocal::DrawStretchPic( const idVec4 & topLeft, const idVec4 & topRight, const idVec4 & bottomRight, const idVec4 & bottomLeft, const idMaterial * material ) {
	...
	idDrawVert * verts;
	if (cstUseShaderParms) {
		cstUseShaderParms = false;
		verts = guiModel->AllocTris(4, quadPicIndexes, 6, material, currentGLState, STEREO_DEPTH_TYPE_NONE, true, cstShaderMsec, cstShaderColor);
	} else {
		verts = guiModel->AllocTris(4, quadPicIndexes, 6, material, currentGLState, STEREO_DEPTH_TYPE_NONE);
	}
	//#modified-fva; END
	if ( verts == NULL ) {
		return;
...
//#modified-fva; BEGIN
void idRenderSystemLocal::CstDrawSmallChar(float x, float y, float wScale, float hScale, int ch) {
	// same as idRenderSystemLocal::DrawSmallChar, but with float coordinates and width/height scale factors
	...
	//if (y < -SMALLCHAR_HEIGHT) {
	if (y < -(float)SMALLCHAR_HEIGHT * hScale) {
		return;
	}
	...
	DrawStretchPic(x, y, SMALLCHAR_WIDTH * wScale, SMALLCHAR_HEIGHT * hScale,
		fcol, frow,
		fcol + size, frow + size,
		charSetMaterial);
}
...
//	primaryWorld = NULL;
	...
	//#modified-fva; BEGIN
	cmd2->cstCanClear = session->GetState() != idSession::LOADING;
	//#modified-fva; END
...
void idRenderSystemLocal::CropRenderSize(int width, int height, bool cstOldStyle) {
//#modified-fva; END
	if ( !R_IsInitialized() ) {
		return;
	}
...
//#modified-fva; BEGIN
void idRenderSystemLocal::CstGenUpdateCinematicCmd(const idMaterial * cinMaterial, int time) {
	if (!cinMaterial || time < 0) {
		return;
	}
	cstUpdateCinematicCommand_t *cmd = (cstUpdateCinematicCommand_t *)R_GetCommandBuffer(sizeof(*cmd));
	cmd->commandId = RC_CST_UPDATE_CINEMATIC;
	cmd->cinMaterial = cinMaterial;
	cmd->time = time;
}

void idRenderSystemLocal::CstUpdateCinematicsNowOutsideRender() {
	emptyCommand_t * cmdHead = frameData->cmdHead;

	for (emptyCommand_t * cmd = cmdHead; cmd; cmd = (emptyCommand_t *)cmd->next) {
		if (cmd->commandId == RC_CST_UPDATE_CINEMATIC) {
			RB_CstUpdateCinematic(cmd);
			cmd->commandId = RC_NOP;
		}
	}
}

void idRenderSystemLocal::CstClearColorBufferNowOutsideRender(float r, float g, float b) {
	GL_SetDefaultState();
	GL_Clear(true, false, false, 0, r, g, b, 1.0f);
}
//#modified-fva; END
