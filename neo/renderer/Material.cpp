...
void idMaterial::ParseStage( idLexer &src, const textureRepeat_t trpDefault ) {
	...
	while ( 1 ) {
		...
		if ( !token.Icmp( "videomap" ) ) {
			...
			*/
			idStr cstFileName = token.c_str();
			ts->cinematic = idCinematic::CstAlloc(cstFileName);
			ts->cinematic->InitFromFile(cstFileName.c_str(), loop);
			//#modified-fva; END
			continue;
		}
...
void idMaterial::UpdateCinematic( int time ) const {
	//#modified-fva; BEGIN
	if (stages && stages[0].texture.cinematic) {
		stages[0].texture.cinematic->CstUpdate(time);
	}
	//#modified-fva; END
}
...
*/
void idMaterial::CstResetCinematicTime_Game(int time) const {
	if (stages && stages[0].texture.cinematic) {
		stages[0].texture.cinematic->CstResetTime_Game(time);
	}
}

void idMaterial::CstResetCinematicTime_Sys(int time) const {
	if (stages && stages[0].texture.cinematic) {
		stages[0].texture.cinematic->CstResetTime_Sys(time);
	}
}
//#modified-fva; END
...
*/
int idMaterial::CstGetCinematicStartTime_Game() const {
	if (stages && stages[0].texture.cinematic) {
		return stages[0].texture.cinematic->CstGetStartTime_Game();
	}
	return -1;
}

int idMaterial::CstGetCinematicStartTime_Sys() const {
	if (stages && stages[0].texture.cinematic) {
		return stages[0].texture.cinematic->CstGetStartTime_Sys();
	}
	return -1;
}
//#modified-fva; END
...
//#modified-fva; BEGIN
bool idMaterial::CstCinematicIsPlaying_Game() {
	if (stages && stages[0].texture.cinematic && stages[0].texture.cinematic->CstIsPlaying_Game()) {
		return true;
	}
	return false;
}

bool idMaterial::CstCinematicIsPlaying_Sys() {
	if (stages && stages[0].texture.cinematic && stages[0].texture.cinematic->CstIsPlaying_Sys()) {
		return true;
	}
	return false;
}

bool idMaterial::CstHasCinematic() const {
	if (stages && stages[0].texture.cinematic) {
		return true;
	}
	return false;
...
int idMaterial::CstCinematicIsTheora() const {
	if (stages && stages[0].texture.cinematic) {
		return stages[0].texture.cinematic->CstIsTheora() ? 1 : 0;
	}
	return -1;
}
//#modified-fva; END
