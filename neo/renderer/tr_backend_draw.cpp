	...
	for ( ; i < numDrawSurfs; i++ ) {
		...
		for ( int stage = 0; stage < shader->GetNumStages(); stage++ ) {
			...
			if ( newStage != NULL ) {
				...
				renderProgManager.CstBindShader(newStage->glslProgram);
				//#modified-fva; END
...
void RB_CstUpdateCinematic(const void *data) {
	cstUpdateCinematicCommand_t * cmd = (cstUpdateCinematicCommand_t *)data;
	if (cmd && cmd->cinMaterial) {
		cmd->cinMaterial->UpdateCinematic(cmd->time);
	}
}
//#modified-fva; END
