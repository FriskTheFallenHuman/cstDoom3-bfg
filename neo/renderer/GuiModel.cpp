...
void idGuiModel::EmitSurfaces( float modelMatrix[16], float modelViewMatrix[16],
	...
	for ( int i = 0; i < surfaces.Num(); i++ ) {
		...
		if ( constRegs ) {
			...
			if (guiSurf.cstUseShaderParms) {
				float cstShaderParms[MAX_ENTITY_SHADER_PARMS];
				memset(cstShaderParms, 0, sizeof(cstShaderParms));
				cstShaderParms[SHADERPARM_RED] = guiSurf.cstShaderColor.x;
				cstShaderParms[SHADERPARM_GREEN] = guiSurf.cstShaderColor.y;
				cstShaderParms[SHADERPARM_BLUE] = guiSurf.cstShaderColor.z;
				cstShaderParms[SHADERPARM_ALPHA] = guiSurf.cstShaderColor.w;
				shader->EvaluateRegisters(regs, cstShaderParms, tr.viewDef->renderView.shaderParms, guiSurf.cstShaderMsec * 0.001f, NULL);
			} else {
				shader->EvaluateRegisters(regs, shaderParms, tr.viewDef->renderView.shaderParms, tr.viewDef->renderView.time[1] * 0.001f, NULL);
			}
			//#modified-fva; END
...
idDrawVert * idGuiModel::AllocTris(int vertCount, const triIndex_t * tempIndexes, int indexCount, const idMaterial * material, const uint64 glState, const stereoDepthType_t stereoType, bool cstUseShaderParms, int cstShaderMsec, const idVec4 &cstShaderColor) {
//#modified-fva; END
	if ( material == NULL ) {
		return NULL;
	}
	...
	if (material != surf->material || glState != surf->glState || stereoType != surf->stereoType
		|| cstUseShaderParms != surf->cstUseShaderParms
		|| (cstUseShaderParms && (cstShaderMsec != surf->cstShaderMsec || cstShaderColor != surf->cstShaderColor))) {
	//#modified-fva; END
		if ( surf->numIndexes ) {
			AdvanceSurf();
		}
		...
		//#modified-fva; BEGIN
		surf->cstUseShaderParms = cstUseShaderParms;
		if (cstUseShaderParms) {
			surf->cstShaderMsec = cstShaderMsec;
			surf->cstShaderColor = cstShaderColor;
		}
		//#modified-fva; END
	}
