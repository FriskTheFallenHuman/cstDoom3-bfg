...
*/
static const int CST_MAX_OVERLAYS_PER_MATERIAL = 16;
static const int CST_OVERLAYS_GRANULARITY = 4;
static const int CST_DEFERRED_OVERLAYS_GRANULARITY = 8;

cstOverlaySurfaces_t::cstOverlaySurfaces_t() :
	material(NULL),
	surfaces(CST_MAX_OVERLAYS_PER_MATERIAL) {
}

idRenderModelOverlay::idRenderModelOverlay() :
	cstOverlays(CST_OVERLAYS_GRANULARITY),
	cstDeferredOverlays(CST_DEFERRED_OVERLAYS_GRANULARITY) {
}
//#modified-fva; END
...
idRenderModelOverlay::~idRenderModelOverlay() {
	...
	*/
	for (int i = 0; i < cstOverlays.Num(); ++i) {
		for (int j = 0; j < cstOverlays[i].surfaces.Num(); ++j) {
			FreeOverlay(cstOverlays[i].surfaces[j]);
		}
	}
	//#modified-fva; END
}
...
void idRenderModelOverlay::CreateOverlay( const idRenderModel *model, const idPlane localTextureAxis[2], const idMaterial *material ) {
	...
	for ( int surfNum = 0; surfNum < model->NumBaseSurfaces(); surfNum++ ) {
		...
		if (tri->staticModelWithJoints != NULL && gCstLatched__r_useGPUSkinning) {
		//#modified-fva; END
		...
		*/
		int cstOverlayIndex = -1;
		for (int i = 0; i < cstOverlays.Num(); ++i) {
			if (cstOverlays[i].material == material) {
				cstOverlayIndex = i;
				break;
			}
		}
		cstOverlaySurfaces_t *cstOverlay = NULL;
		if (cstOverlayIndex >= 0) {
			cstOverlay = &cstOverlays[cstOverlayIndex];
			if (cstOverlay->surfaces.Num() == CST_MAX_OVERLAYS_PER_MATERIAL) {
				// remove the oldest surface to make room for the new one
				FreeOverlay(cstOverlay->surfaces[0]);
				cstOverlay->surfaces.RemoveIndex(0);
			}
		} else {
			cstOverlay = &cstOverlays.Alloc();
			cstOverlay->material = material;
		}
		overlay_t &overlay = cstOverlay->surfaces.Alloc();
		//#modified-fva; END
...
void idRenderModelOverlay::CreateDeferredOverlays( const idRenderModel * model ) {
	...
	nextDeferredOverlay = 0;
	*/
	if (cstDeferredOverlays.Num()) {
		for (int i = 0; i < cstDeferredOverlays.Num(); ++i) {
			const overlayProjectionParms_t &parms = cstDeferredOverlays[i];
			CreateOverlay(model, parms.localTextureAxis, parms.material);
		}
		cstDeferredOverlays.Clear();
	}
	//#modified-fva; END
}
...
void idRenderModelOverlay::AddDeferredOverlay( const overlayProjectionParms_t & localParms ) {
	...
	*/
	cstDeferredOverlays.Append(localParms);
	//#modified-fva; END
}
...
drawSurf_t * idRenderModelOverlay::CreateOverlayDrawSurf( const viewEntity_t *space, const idRenderModel *baseModel, unsigned int index ) {
	...
	if ((int)index >= cstOverlays.Num() || cstOverlays[index].surfaces.Num() == 0) {
	//#modified-fva; END
		return NULL;
	}
	...
	*/
	cstOverlaySurfaces_t &cstOverlay = cstOverlays[index];
	for (int i = 0; i < cstOverlay.surfaces.Num(); ++i) {
		maxVerts += cstOverlay.surfaces[i].numVerts;
		maxIndexes += cstOverlay.surfaces[i].numIndexes;
	}
	//#modified-fva; END
	...
	*/
	for (int i = 0; i < cstOverlay.surfaces.Num(); ++i) {
		overlay_t &overlay = cstOverlay.surfaces[i];
		if (overlay.numVerts == 0) {
			continue;
		}
	//#modified-fva; END
		...
		if ( baseSurf == NULL || baseSurf->id != overlay.surfaceId ) {
			...
			if ( staticModel->FindSurfaceWithId( overlay.surfaceId, overlay.surfaceNum ) ) {
				...
				*/
				cstOverlay.surfaces.RemoveIndex(i);
				--i;
				//#modified-fva; END
				continue;
			}
	...
	R_SetupDrawSurfShader(drawSurf, cstOverlay.material, &space->entityDef->parms);
	//#modified-fva; END
