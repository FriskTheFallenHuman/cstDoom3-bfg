...
//#modified-fva; BEGIN
idCVar cst_shadersPrintCompileIssues("cst_shadersPrintCompileIssues", "2", CVAR_RENDERER | CVAR_INTEGER | CVAR_NOCHEAT, "print shader compilation errors/warnings:\n  0 = don't print\n  1 = print using original behavior\n  2 = print only when compilation fails", 0, 2);
//#modified-fva; END
...
attribInfo_t attribsPC[] = {
	...
	// fragment program output
	{ "float4",		"color",		"COLOR",		"cst_FragColor",		0,	AT_PS_OUT,	0 }, // gl_FragColor is still available in glsl 1.5, but it is deprecated; using a custom name for color output is preferred
	{ "half4",		"hcolor",		"COLOR",		"cst_FragColor",		0,	AT_PS_OUT,	0 },
	{ "float4",		"color0",		"COLOR0",		"cst_FragColor",		0,	AT_PS_OUT,	0 },
	{ "float4",		"color1",		"COLOR1",		"cst_FragColor",		1,	AT_PS_OUT,	0 },
	{ "float4",		"color2",		"COLOR2",		"cst_FragColor",		2,	AT_PS_OUT,	0 },
	{ "float4",		"color3",		"COLOR3",		"cst_FragColor",		3,	AT_PS_OUT,	0 },
	{ "float",		"depth",		"DEPTH",		"gl_FragDepth",			4,	AT_PS_OUT,	0 },

	// vertex to fragment program pass through
	{ "float4",		"color",		"COLOR",		"cst_Color",			0,	AT_VS_OUT,	0 }, // gl_FrontColor and gl_FrontSecondaryColor aren't available in the core profile of glsl 1.5, and a custom variable is preferred for pass through
	{ "float4",		"color0",		"COLOR0",		"cst_Color",			0,	AT_VS_OUT,	0 },
	{ "float4",		"color1",		"COLOR1",		"cst_SecondaryColor",	0,	AT_VS_OUT,	0 },


	{ "float4",		"color",		"COLOR",		"cst_Color",			0,	AT_PS_IN,	0 }, // gl_Color and gl_SecondaryColor aren't available in the core profile of glsl 1.5, and a custom variable is preferred for pass through
	{ "float4",		"color0",		"COLOR0",		"cst_Color",			0,	AT_PS_IN,	0 },
	{ "float4",		"color1",		"COLOR1",		"cst_SecondaryColor",	0,	AT_PS_IN,	0 },

	{ "half4",		"hcolor",		"COLOR",		"cst_Color",			0,	AT_PS_IN,	0 },
	{ "half4",		"hcolor0",		"COLOR0",		"cst_Color",			0,	AT_PS_IN,	0 },
	{ "half4",		"hcolor1",		"COLOR1",		"cst_SecondaryColor",	0,	AT_PS_IN,	0 },
	...
	{ "half4",		"htexcoord8",	"TEXCOORD8",	"vofi_TexCoord8",		0,	AT_PS_IN,	0 },
	{ "half4",		"htexcoord9",	"TEXCOORD9",	"vofi_TexCoord9",		0,	AT_PS_IN,	0 },
	{ "float",		"fog",			"FOG",			"cst_FogFragCoord",		0,	AT_VS_OUT,	0 }, // gl_FogFragCoord isn't available in the core profile of glsl 1.5
	{ "float4",		"fog",			"FOG",			"cst_FogFragCoord",		0,	AT_PS_IN,	0 },
	{ NULL,			NULL,			NULL,			NULL,					0,	0,			0 }
};
//#modified-fva; END
	...
	while ( src.ReadToken( &token ) ) {
		...
		if ( token == "struct" ) {
			if ( src.CheckTokenString( "VS_IN" ) ) {
				...
				//#modified-fva; BEGIN
				// add the built-in gl_* fragment shader inputs; these shouldn't be declared
				inOutVariable_t var;
...
GLuint idRenderProgManager::LoadGLSLShader( GLenum target, const char * name, idList<int> & uniforms ) {
	...
	if ( ( glslFileLength <= 0 ) || ( hlslTimeStamp > glslTimeStamp ) ) {
		...
		*/
		...
		idStr cstOutFileHLSL(outFileHLSL);
		idStr cstOutFileGLSL(outFileGLSL);
		idStr cstOutFileUniforms(outFileUniforms);

		cstOutFileHLSL.ToLower();
		cstOutFileGLSL.ToLower();
		cstOutFileUniforms.ToLower();

		fileSystem->WriteFile(cstOutFileHLSL.c_str(), programHLSL.c_str(), programHLSL.Length(), "fs_basepath");
		fileSystem->WriteFile(cstOutFileGLSL.c_str(), programGLSL.c_str(), programGLSL.Length(), "fs_basepath");
		if (r_useUniformArrays.GetBool()) {
			fileSystem->WriteFile(cstOutFileUniforms.c_str(), programUniforms.c_str(), programUniforms.Length(), "fs_basepath");
		}
		//#modified-fva; END
	} else {
	...
	if ( shader ) {
		...
		*/
		...
		//#modified-fva; BEGIN
		if (cst_shadersPrintCompileIssues.GetInteger() == 1 || (cst_shadersPrintCompileIssues.GetInteger() == 2 && compiled == GL_FALSE)) {
			int infologLength = 0;
			qglGetShaderiv(shader, GL_INFO_LOG_LENGTH, &infologLength);
			if (infologLength > 1) {
				idTempArray<char> infoLog(infologLength);
...
void idRenderProgManager::LoadGLSLProgram( const int programIndex, const int vertexShaderIndex, const int fragmentShaderIndex ) {
	...
	if ( program ) {
		...
		cst_qglBindFragDataLocation(program, 0, "cst_FragColor");
		//#modified-fva; END
