...
//#modified-fva; BEGIN
idCVar cst_preloadGuis("cst_preloadGuis", "0", CVAR_SYSTEM | CVAR_BOOL | CVAR_NOCHEAT, "preload binary gui files");
//#modified-fva; END
...
bool idUserInterfaceLocal::InitFromFile( const char *qpath, bool rebuild, bool cache ) {
	...
	for ( int i = 0; i < 2; i++ ) {
		...
		if ( i == 0 ) {
			...
			trySource.Replace("guis/", va("guis/%s/", common->CstGetLatchedLanguage().c_str()));
			//#modified-fva; END
		}
...
//#modified-fva; BEGIN
void idUserInterfaceLocal::CstRequestCinematicsUpdate(int time) {
	if (desktop) {
		desktop->CstRequestCinematicsUpdate(time);
	}
}

bool idUserInterfaceLocal::CstHasCinematics() {
	if (desktop && desktop->CstHasCinematics()) {
		return true;
	}
	return false;
}
...
void idUserInterfaceLocal::RecurseSetKeyBindingNames( idWindow *window ) {
	...
	if ( v ) {
		...
		SetStateString(v->GetName(), idKeyInput::KeysFromBinding(v->GetName(), 0, 0));
		//#modified-fva; END
	}
	i = 0;
