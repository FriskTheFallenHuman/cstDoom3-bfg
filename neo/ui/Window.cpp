...
void idWindow::StateChanged( bool redraw ) {
	...
	//#modified-fva; BEGIN
	if (redraw && (flags & WIN_DESKTOP) && r_skipGuiShaders.GetInteger() != 3) {
...
void idWindow::Redraw(float x, float y, bool hud) {
	...
	//#modified-fva; BEGIN
	//if (!visible) {
	if (!visible || rect.w() == 0.0f || rect.h() == 0.0f) {
	//#modified-fva; END
		return;
	}
...
void idWindow::ResetCinematics() {
	if ( background ) {
		...
		background->CstResetCinematicTime_Game(gui->GetTime());
		//#modified-fva; END
	}
}

//#modified-fva; BEGIN
void idWindow::CstRequestCinematicsUpdate(int time) {
	for (int i = 0; i < drawWindows.Num(); ++i) {
		if (drawWindows[i].win) {
			drawWindows[i].win->CstRequestCinematicsUpdate(time);
		} else {
			drawWindows[i].simp->CstRequestCinematicsUpdate(time);
		}
	}
	if (background && background->CstHasCinematic() && common->Game()) {
		common->Game()->CstRequestCinematicUpdate(background, time);
	}
}

bool idWindow::CstHasCinematics() {
	if (background && background->CstHasCinematic()) {
		return true;
	}
	for (int i = 0; i < drawWindows.Num(); ++i) {
		if (drawWindows[i].win) {
			if (drawWindows[i].win->CstHasCinematics()) {
				return true;
			}
		} else {
			if (drawWindows[i].simp->CstHasCinematics()) {
				return true;
			}
		}
	}
...
void idWindow::WriteToSaveGame( idFile *savefile ) {
	...
	if ( background ) {
		...
		savefile->WriteInt(background->CstGetCinematicStartTime_Game());
		//#modified-fva; END
	} else {
		savefile->WriteInt( -1 );
	}
...
//	}
	...
	if ( background ) {
		...
		background->CstResetCinematicTime_Game(cinematicStartTime);
		//#modified-fva; END
	}
