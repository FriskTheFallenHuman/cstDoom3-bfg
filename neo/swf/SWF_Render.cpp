...
void idSWF::Render( idRenderSystem * gui, int time, bool isSplitscreen ) {
	...
	if (!cstFreeze && swf_timescale.GetFloat() > 0.0f) {
	//#modified-fva; END
		if ( lastRenderTime == 0 ) {
			lastRenderTime = currentTime;
			framesToRun = 1;
	...
	if ( isMouseInClientArea && ( mouseEnabled && useMouse ) && ( InhibitControl() || ( !InhibitControl() && !useInhibtControl ) ) ) {
		...
		*/
		const float cstMouseSize = 32.0f * scale;
		if (!hasHitObject) {
			DrawStretchPic(mouse.x, mouse.y, cstMouseSize, cstMouseSize, 0, 0, 1, 1, guiCursor_arrow);
		} else {
			DrawStretchPic(mouse.x, mouse.y, cstMouseSize, cstMouseSize, 0, 0, 1, 1, guiCursor_hand);
		}
		//#modified-fva; END
	}
...
void idSWF::RenderEditText( idRenderSystem * gui, idSWFTextInstance * textInstance, const swfRenderState_t & renderState, int time, bool isSplitscreen ) {
	...
	while ( charIndex < text.Length() ) {
		...
		//#modified-fva; BEGIN
		if (!textInstance->ignoreColor && (idStr::IsColor(text.c_str() + glyphStart) || (glyphStart > 0 && idStr::IsColor(text.c_str() + glyphStart - 1)))) {
			glyphSkip = 0.0f;
		}
		//#modified-fva; END
				...
				if ( shape->flags & ( SWF_ET_MULTILINE | SWF_ET_WORDWRAP ) ) {
					if ( lastbreak > 0 ) {
						...
						if ( maxLines == 1 ) {
							...
							bool cstFound = false;
							int cstCapIdx = currentLine->Length() - 1;
							int cstRemaining = 3;
							int cstMaxLen = 1;
							for (; cstCapIdx >= 0; --cstCapIdx) {
								if (idStr::IsValidUTF8(currentLine->c_str() + cstCapIdx, cstMaxLen)) {
									--cstRemaining;
									if (cstRemaining == 0) {
										cstFound = true;
										break;
							...
							if (cstFound) {
								if (!textInstance->ignoreColor && cstCapIdx > 0 && idStr::IsColor(currentLine->c_str() + cstCapIdx - 1)) {
									--cstCapIdx;
								}
								currentLine->CapLength(cstCapIdx);
							} else {
								currentLine->CapLength(0);
							}
							//#modified-fva; END
							currentLine->Append( "..." );
							break;
...
void idSWF::FindTooltipIcons( idStr * text ) {
	...
	//for ( userCmdString_t * ucs = userCmdStrings ; ucs->string ; ucs++ ) {
		...
		if ( ucs.string && idStr::FindText( text->c_str(), ucs.string, false ) != idStr::INVALID_POSITION ) {
			...
			keyBindings_t bind = idKeyInput::KeyBindingsFromBinding(ucs.string, 0, 0, true);
			//#modified-fva; END
			idStr gamepad = "<";
