...
public:
	...
	void			WriteS16(int16 value);
	void			WriteS32(int32 value);
	void			WriteFloat(float value);
	void			WriteDouble(double value);
	void			WriteString(const char *str);
	void			WriteMatrix(const swfMatrix_t &matrix);
	void			WriteColorXFormRGBA(const swfColorXform_t &cxf);

	int				Length();
	int				Tell();
	void			Seek(int byteIndex);
	const byte *	Ptr();

private:
	void			ByteAlign();
	void			WriteBits(uint32 value, unsigned numBits);
	bool			ReadByte(byte &value, byte _default = 0);
	void			WriteByte(byte value, bool advance = true);

	int				BitsForUnsigned(uint32 value);
	int				BitsForSigned(int32 value);
	int32			ToFixed16(float value); // to fixed point 16.16
	int16			ToFixed8(float value); // to fixed point 8.8
	int32			ToTwips(float value);
