...
float idSWFTextInstance::GetTextLength() {
	...
	if ( verify( swf != NULL ) ) {
		...
		while ( index < tlen ) {
			//#modified-fva; BEGIN
			if (!ignoreColor && idStr::IsColor(txtLengthCheck.c_str() + index)) {
				index += 2;
				continue;
			}
			//#modified-fva; END
...
idStr idSWFTextInstance::GetParagraphText( int time ) {
	if ( triggerGenerate ) {
		...
		*/
		while (time - rndTime >= renderDelay) {
			rndTime += renderDelay;
			needsSoundUpdate = true;
...
int idSWFTextInstance::CalcMaxScroll( int numLines ) {
	...
	while ( charIndex < textCheck.Length() ) {
		...
		//#modified-fva; BEGIN
		if (!ignoreColor && (idStr::IsColor(textCheck.c_str() + glyphStart) || (glyphStart > 0 && idStr::IsColor(textCheck.c_str() + glyphStart - 1)))) {
			glyphSkip = 0.0f;
		}
		//#modified-fva; END
			...
			if ( shape->flags & ( SWF_ET_MULTILINE | SWF_ET_WORDWRAP ) ) {
				if ( lastbreak > 0 ) {
					...
					if ( maxLines == 1 ) {
						...
						if (cstFound) {
							if (!ignoreColor && cstCapIdx > 0 && idStr::IsColor(currentLine->c_str() + cstCapIdx - 1)) {
								--cstCapIdx;
							}
