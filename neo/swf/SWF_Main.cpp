...
//#modified-fva; BEGIN
idCVar cst_swfLoadJson("cst_swfLoadJson", "0", CVAR_SYSTEM | CVAR_BOOL | CVAR_NOCHEAT, "load contents from json instead of swf");
idCVar cst_swfExportAsJson("cst_swfExportAsJson", "0", CVAR_SYSTEM | CVAR_BOOL | CVAR_NOCHEAT, "export swf contents as json when loaded");
idCVar cst_swfExportAtlas("cst_swfExportAtlas", "0", CVAR_SYSTEM | CVAR_BOOL | CVAR_NOCHEAT, "export altlas images as tga when loaded");
//#modified-fva; END
...
//#modified-fva; BEGIN
idSWF::CstMouseInClientAreaFilter idSWF::cstMouseInClientAreaFilter;
//#modified-fva; END
...
idSWF::idSWF( const char * filename_, idSoundWorld * soundWorld_ ) {
	...
	if (cst_swfLoadJson.GetBool()) {
		idStr jsonFileName = filename;
		jsonFileName.SetFileExtension(".json");
		timestamp = fileSystem->GetTimestamp(jsonFileName.c_str());
	} else {
		timestamp = fileSystem->GetTimestamp(filename);
	}
	//#modified-fva; END
	...
	*/
	if (swf_loadBinary.GetBool()) {
		if (!LoadBinary(binaryFileName, timestamp)) {
			if (cst_swfLoadJson.GetBool() ? CstLoadJson(filename) : LoadSWF(filename)) {
				WriteBinary(binaryFileName);
			}
		}
	} else {
		cst_swfLoadJson.GetBool() ? CstLoadJson(filename) : LoadSWF(filename);
	}
	//#modified-fva; END
	...
	//#modified-fva; BEGIN
	if (cst_swfExportAsJson.GetBool()) {
		CstWriteJson(filename);
	}
	if (cst_swfExportAtlas.GetBool()) {
		CstExportAtlas(atlasFileName);
	}
	//#modified-fva; END
