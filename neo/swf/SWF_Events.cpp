...
//#modified-fva; BEGIN
class CstScriptObjectGuard {
public:
	CstScriptObjectGuard(idSWFScriptObject *_scriptObject) : scriptObject(_scriptObject) {
		if (scriptObject) {
			scriptObject->AddRef();
		}
	}
	~CstScriptObjectGuard() {
		if (scriptObject) {
	...
	if ( event->evType == SE_KEY ) {
		if ( event->evValue == K_MOUSE1 ) {
			...
			if ( event->evValue2 ) {
				...
				//#modified-fva; BEGIN
				CstScriptObjectGuard cstHitObjectGuard(hitObject);
				//#modified-fva; END
				if ( hitObject != NULL ) {
		...
		for ( int runaway = 0; runaway < 32; runaway++ ) {
			...
			if ( useFunction.IsFunction() && event->evValue2 ) {
				...
				const char * action = idKeyInput::GetBinding(event->evValue, 0, 0);
				//#modified-fva; END
		...
		if (!isMouseInClientArea) {
			CstMouseInClientAreaFilter &filter = cstMouseInClientAreaFilter;
			if (filter.showMouse
				|| (event->evType == SE_MOUSE_ABSOLUTE && filter.xyValid && (filter.x != event->evValue || filter.y != event->evValue2))
				|| (event->evType == SE_MOUSE && (event->evValue != 0 || event->evValue2 != 0))) {
				isMouseInClientArea = true;
				filter.showMouse = true;
			}
			if (event->evType == SE_MOUSE_ABSOLUTE) {
				filter.xyValid = true;
				filter.x = event->evValue;
				filter.y = event->evValue2;
			}
		}
		//#modified-fva; END
