...
void idCommonLocal::ExecuteMapChange() {
	...
	if ( mapSpawnData.savegameFile ) {
		...
		renderSystem->CstUpdateCinematicsNowOutsideRender();
		//#modified-fva; END
	} else {
		if ( !IsMultiplayer() ) {
	...
	mapSpawned = true;
	//#modified-fva; BEGIN
	// this avoids evaluating both SE_CST_KEY and SE_KEY events when a loadgame is issued from F1-F12 while the menus are active
...
bool idCommonLocal::SaveGame( const char * saveName ) {
	...
	//#modified-fva; BEGIN
	if (game->Cst_Shell_IsGameComplete()) {
	...
	gameDetails.descriptors.Set(SAVEGAME_DETAIL_FIELD_LANGUAGE, CstGetLatchedLanguage().c_str());
	//#modified-fva; END
...
bool idCommonLocal::LoadGame( const char * saveName ) {
	...
	for ( int i = 0; i < sgdl.Num(); i++ ) {
		if ( sgdl[i].slotName == saveName ) {
			...
			if (sgdl[i].GetLanguage() != CstGetLatchedLanguage()) {
			//#modified-fva; END
...
void idCommonLocal::OnLoadFilesCompleted( idSaveLoadParms & parms ) {
	if ( ( mapSpawnData.savegameFile != NULL ) && ( parms.GetError() == SAVEGAME_E_NONE ) ) {
		...
		if ((gamename != GAME_NAME) || (mapname.IsEmpty()) || (parms.description.GetSaveVersion() != CST_SAVEGAME_VERSION)) {
		//#modified-fva; END
