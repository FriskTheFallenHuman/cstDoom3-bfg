...
//#modified-fva; BEGIN
const int CST_ECHO_NUM_MSGS = 3;
const int CST_ECHO_NUM_UI_ITEMS = CST_ECHO_NUM_MSGS + 1;

struct CstEchoUiItemPtrs {
	idSWFSpriteInstance * labelSprite;
	idSWFTextInstance * labelText;
	idSWFSpriteInstance * backgroundSprite;
};
...
public:
	...
	virtual const char *		KeysFromBinding(const char *bind, int cstLayer, int cstDomain);
	virtual const char *		BindingFromKey(const char *key, int cstLayer, int cstDomain);
	//#modified-fva; END
...
private:
	idSWF *				cstEchoUi;
	CstEchoUiItemPtrs	cstEchoUiItems[CST_ECHO_NUM_UI_ITEMS];
	CstEchoMsg			cstEchoMsgAchiev[CST_ECHO_NUM_MSGS];
	int					cstCurrentEchoMsgAchiev;
	CstEchoMsg			cstEchoMsg[CST_ECHO_NUM_MSGS];
	int					cstCurrentEchoMsg;

	static idCVar cst_achievementsNotifyTime;
	static idCVar cst_achievementsNoPrint;

	static idCVar cst_notifyTime;
	static idCVar cst_noPrint;

	bool cstWasAutomapActive;
	bool cstServerDedicated;
	bool cstSkipCinematicRequested;
	bool cstCinematicSkipped;
	idStrList cstInstalledLanguages;
	idStr cstLatchedLanguage;
	bool cstShowDefaultedFullscreenMsgOnInit;
	std::atomic<bool> cstBrtEnableRequested; // brt = backend render tools
	bool cstBrtEnabled;
	bool cstQuitRequested;

	void CstSetupLanguage();
	void CstAddDefaultedFullscreenDialogIfNeeded();
	void CstSetEnvForBackendRendertools();
	void CstEchoCmdInit();
	void CstEchoCmdShutdown();
	void CstEchoUiInit();
	void CstEchoUiShutdown();
	void CstEchoDraw();

public:
	virtual void CstPrintAchievementMsg(const idStr & in_msg);
	virtual void CstPrintMsg(const idStr & in_msg);
	virtual void CstClearMsgs(bool keepAchievementMessages);
	virtual bool CstGetServerDedicated() { return cstServerDedicated; }
	virtual void CstSetServerDedicated(bool dedicated) { cstServerDedicated = dedicated; }
	virtual void CstSetCinematicSkipped() { cstCinematicSkipped = true; }
	virtual const idStrList & CstGetInstalledLanguages() { return cstInstalledLanguages; }
	virtual const idStr & CstGetLatchedLanguage() { return cstLatchedLanguage; }
	virtual void CstSetShowDefaultedFullscreenMsgOnInit() { cstShowDefaultedFullscreenMsgOnInit = true; }
	virtual void CstBrtRequestEnable() { cstBrtEnableRequested.store(true, std::memory_order_relaxed); } // brt = backend render tools
	virtual bool CstBrtAreEnabled() { return cstBrtEnabled; }
	virtual void CstSetQuitRequested(bool quitRequested) { cstQuitRequested = quitRequested; }
	//#modified-fva; END
};
