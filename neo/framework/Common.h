...
public:
	...
	virtual const char *		KeysFromBinding(const char *bind, int cstLayer, int cstDomain) = 0;
	//#modified-fva; END

								// Returns the binding bound to the key
	//#modified-fva; BEGIN
	//virtual const char *		BindingFromKey( const char *key ) = 0;
	virtual const char *		BindingFromKey(const char *key, int cstLayer, int cstDomain) = 0;
	//#modified-fva; END
	...
	//#modified-fva; BEGIN
	virtual void CstPrintAchievementMsg(const idStr & in_msg) = 0;
	virtual void CstPrintMsg(const idStr & in_msg) = 0;
	virtual void CstClearMsgs(bool keepAchievementMessages) = 0;
	virtual bool CstGetServerDedicated() = 0;
	virtual void CstSetServerDedicated(bool dedicated) = 0;
	virtual void CstSetCinematicSkipped() = 0;
	virtual const idStrList & CstGetInstalledLanguages() = 0;
	virtual const idStr & CstGetLatchedLanguage() = 0;
	virtual void CstSetShowDefaultedFullscreenMsgOnInit() = 0;
	virtual void CstBrtRequestEnable() = 0; // brt = backend render tools
	virtual bool CstBrtAreEnabled() = 0;
	virtual void CstSetQuitRequested(bool quitRequested) = 0;
	//#modified-fva; END
};
