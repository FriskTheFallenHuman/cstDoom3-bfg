...
idInternalCVar::idInternalCVar( const char *newName, const char *newValue, int newFlags ) {
	...
	//#modified-fva; BEGIN
	cstValueLength = valueString.Length();
	//#modified-fva; END
...
private:
	...
	//#modified-fva; BEGIN
	static void				CstToggleAlwaysRun_f(const idCmdArgs &args);
	static void				CstDcToggleAlwaysRun_f(const idCmdArgs &args);
	//#modified-fva; END
};
...
void idCVarSystemLocal::Init() {
	...
	//#modified-fva; BEGIN
	cmdSystem->AddCommand("cstToggleAlwaysRun", CstToggleAlwaysRun_f, CMD_FL_SYSTEM, "toggles in_alwaysRun in Doom 3");
	cmdSystem->AddCommand("cstDcToggleAlwaysRun", CstDcToggleAlwaysRun_f, CMD_FL_SYSTEM, "toggles cst_dcAlwaysRun in Doom 1 & 2");
	//#modified-fva; END
...
//#modified-fva; BEGIN
void idCVarSystemLocal::CstToggleAlwaysRun_f(const idCmdArgs &args) {
	if (common->GetCurrentGame() != DOOM3_BFG) {
		return;
	}

	idInternalCVar *cvar = localCVarSystem.FindInternal("in_alwaysRun");
	if (!cvar) {
		return;
	}
	...
	idStr str;
	if (value) {
		str = idLocalization::GetString(CST_STR_ALWAYS_RUN_ON);
	} else {
		str = idLocalization::GetString(CST_STR_ALWAYS_RUN_OFF);
	}
	common->CstPrintMsg(str);
}

// ===============
void idCVarSystemLocal::CstDcToggleAlwaysRun_f(const idCmdArgs &args) {
	if (common->GetCurrentGame() == DOOM3_BFG) {
		return;
	}

	idInternalCVar *cvar = localCVarSystem.FindInternal("cst_dcAlwaysRun");
	if (!cvar) {
		return;
	}
	bool value = !cvar->GetBool();
	...
	if (g && !g->menuactive && !g->demoplayback && (g->viewactive || g->automapactive) && !g->paused) { // if ingame and not paused
		if (value) {
			g->plyr->message = "Always Run ON";
		} else {
			g->plyr->message = "Always Run OFF";
		}
	}
