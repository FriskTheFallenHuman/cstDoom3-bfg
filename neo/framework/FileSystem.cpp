...
//#modified-fva; BEGIN
#define FSFLAG_CST_ENSURE_FILE_MEM (1 << 2)
//#modified-fva; END
...
public:
	...
	//#modified-fva; BEGIN
	virtual idFile *		CstOpenFileRead_FileMemory(const char *relativePath, bool allowCopyFiles = true, const char* gamedir = NULL);
	//#modified-fva; END
	...
	idFile *				GetResourceFile(const char *fileName, unsigned int cst_memFile); // cst_memFile: 0 = no, 1 = normal, 2 = always idFile_Memory
	//#modified-fva; END
	...
	//#modified-fva; BEGIN
	static void				CstWriteResourceFile_f(const idCmdArgs &args);
	//#modified-fva; END
...
idCVar  idFileSystemLocal::fs_game_base("fs_game_base", "cst_d3", CVAR_SYSTEM | CVAR_INIT | CVAR_SERVERINFO, "alternate mod path, searched after the main fs_game path, before the basedir");
//#modified-fva; END
...
idCVar	fs_resourceLoadPriority("fs_resourceLoadPriority", "1", CVAR_SYSTEM | CVAR_INTEGER | CVAR_NOCHEAT, "if 1, open requests will be honored from resource files first; if 0, the resource files are checked after normal search paths", 0, 1);
//#modified-fva; END
...
//#modified-fva; BEGIN
idCVar cst_exportPath("cst_exportPath", "_exported", CVAR_SYSTEM | CVAR_INIT, "base folder for exported files");
idCVar cst_resourcesAddAll("cst_resourcesAddAll", "1", CVAR_SYSTEM | CVAR_INIT | CVAR_BOOL, "add all resource files on startup");
//#modified-fva; END
...
void idFileSystemLocal::BeginLevelLoad( const char *name, char *_blockBuffer, int _blockBufferSize ) {
	...
	//}
	if (!cst_resourcesAddAll.GetBool() && resourceFiles.Num() > 0) {
		AddResourceFile(va("%s.resources", manifestName.c_str()));
	}
	//#modified-fva; END
...
int idFileSystemLocal::ReadFile( const char *relativePath, void **buffer, ID_TIME_T *timestamp ) {
	...
	*/
	// take fs_resourceLoadPriority into account
	if (buffer == NULL && timestamp != NULL && resourceFiles.Num() > 0 && fs_resourceLoadPriority.GetInteger() == 1) {
		static idResourceCacheEntry rc;
		int size = 0;
		if (GetResourceCacheEntry(relativePath, rc)) {
			*timestamp = 0;
			size = rc.length;
			return size; // <-- moved here; check the filesystem if not found
		}
	}
	//#modified-fva; END
	...
	if ( f == NULL ) {
		//#modified-fva; BEGIN
		if (buffer == NULL && timestamp != NULL && resourceFiles.Num() > 0 && fs_resourceLoadPriority.GetInteger() == 0) {
			static idResourceCacheEntry rc;
			int size = 0;
			if (GetResourceCacheEntry(relativePath, rc)) {
				*timestamp = 0;
	...
	if ( resourceFiles.Num() > 0 ) {
		...
		while ( idx >= 0 ) {
			for ( int i = 0; i < resourceFiles[ idx ]->cacheTable.Num(); i++ ) {
				...
				if (rt.filename[pathLength] == '\0' || (pathLength > 0 && rt.filename[pathLength - 1] != '/')) {
				//#modified-fva; END
					continue;
				}
...
void idFileSystemLocal::ExtractResourceFile_f( const idCmdArgs &args ) {
	...
	bool copyWaves = args.Argc() > 3 && atoi(args.Argv(3)) == 1;
	//#modified-fva; END
	idResourceContainer::ExtractResourceFile( filename, outPath, copyWaves );
}

//#modified-fva; BEGIN
void GetFileList(const char* dir, const char* ext, idStrList& list); // this is in "neo/framework/Common_localize.cpp"

void idFileSystemLocal::CstWriteResourceFile_f(const idCmdArgs &args) {
	if (args.Argc() != 3) {
		common->Printf("Usage: cstWriteResourceFile <inFolderFullPath> <outFileName>\n");
		return;
	}
	idStr inFolderPath = args.Argv(1);
	idStr outFileName = args.Argv(2);

	inFolderPath.BackSlashesToSlashes();
	inFolderPath.StripTrailing('/');
	outFileName.StripFileExtension();

	idStrList inFilePaths;
	::GetFileList(inFolderPath.c_str(), ".*", inFilePaths);
	if (inFilePaths.Num() == 0) {
		common->Printf("Input folder is empty.\n");
		return;
	}

	for (int i = 0; i < inFilePaths.Num(); ++i) {
		inFilePaths[i].BackSlashesToSlashes();
	}
	inFilePaths.SortWithTemplate(idSort_PathStr());
	idStr stripPath = inFolderPath + '/';

	// ==========
	// code below is adapted from idResourceContainer::WriteResourceFile and from idFileSystemLocal::OpenFileReadFlags
	// ==========
	...
	for (int i = 0; i < inFilePaths.Num(); i++) {
		flist.Append(inFilePaths[i]);
		idFile *file = fileSystem->OpenExplicitFileRead(inFilePaths[i].c_str());
		if (file) {
			size += file->Length();
	...
	for (int idx = 0; idx < outPutFiles.Num(); idx++) {
		...
		idStr fileName = outFileName;
		if (idx > 0) {
			fileName = va("%s_%02d", outFileName.c_str(), idx);
		}
		fileName.SetFileExtension("resources");

		idFile *resFile = fileSystem->OpenFileWrite(fileName.c_str());
		if (resFile == NULL) {
			common->Warning("Cannot open %s for writing.\n", fileName.c_str());
			return;
		}

		common->Printf("Writing resource file %s\n", fileName.c_str());
		...
		for (int i = 0; i < fileList.Num(); i++) {
			...
			ent.filename.StripLeadingOnce(stripPath.c_str()); // path relative to input folder
			ent.length = 0;
			ent.offset = 0;

			idFile *file = fileSystem->OpenExplicitFileRead(fileList[i].c_str());
			if (!file) {
				continue;
			}
			idFile_Memory *fm = new (TAG_IDFILE) idFile_Memory(ent.filename.c_str());
			fm->SetLength(file->Length());
			file->Read((void *)fm->GetDataPtr(), file->Length());
			fileSystem->CloseFile(file);
			fm->TakeDataOwnership();
...
void idFileSystemLocal::CreateCRCsForResourceFileList( const idFileList & list ) {
	for ( int fileIndex = 0; fileIndex < list.GetNumFiles(); ++fileIndex ) {
		...
		std::auto_ptr<idFile_Memory> currentFile(static_cast<idFile_Memory *>(fileSystem->CstOpenFileRead_FileMemory(list.GetFile(fileIndex))));
		//#modified-fva; END
		...
		if ( crcOutputFile.get() == NULL ) {
			...
			// This fixes warning C4840: non-portable use of class 'idStr' as an argument to a variadic function.
			idLib::Printf("Error writing CRC file %s.\n", crcFilename.c_str());
			//#modified-fva; END
			continue;
		}
...
void idFileSystemLocal::AddGameDirectory( const char *path, const char *dir ) {
	...
	*/
	struct local {
		static void AddResources(idFileSystemLocal &fs, const idStr &rootDir, const idStr &gameDir, const idStr &subDir) {
			idStr resDir = fs.BuildOSPath(rootDir.c_str(), gameDir.c_str(), subDir.c_str());
			if (subDir.IsEmpty()) {
				resDir.CapLength(resDir.Length() - 1); // remove trailing slash
			}
			idStrList resFileNames;
			fs.ListOSFiles(resDir.c_str(), ".resources", resFileNames);
			resFileNames.SortWithTemplate(idSort_PathStr());
			for (int i = 0; i < resFileNames.Num(); ++i) {
				idStr file;
				if (subDir.IsEmpty()) {
					file = resFileNames[i];
				} else {
					file = subDir + '/' + resFileNames[i];
				}
				idResourceContainer *rc = new idResourceContainer();
				if (!rc) {
					continue;
				}
				if (!rc->Init(file.c_str(), fs.resourceFiles.Num())) {
					delete rc;
					rc = NULL;
					continue;
				}
				fs.resourceFiles.Append(rc);
				common->Printf("Loaded resource file %s\n", file.c_str());
			}
		}
	};
	local::AddResources(*this, path, dir, "");
	if (cst_resourcesAddAll.GetBool()) {
		local::AddResources(*this, path, dir, "maps");
	}
	//#modified-fva; END
}
...
void idFileSystemLocal::Startup() {
	...
	//#modified-fva; BEGIN
	cmdSystem->AddCommand("cstWriteResourceFile", CstWriteResourceFile_f, CMD_FL_SYSTEM, "writes a .resources file from the contents of the specified folder");
	//#modified-fva; END
...
void idFileSystemLocal::Init() {
	...
	//#modified-fva; BEGIN
	common->StartupVariable("cst_exportPath");
	common->StartupVariable("cst_resourcesAddAll");
	//#modified-fva; END
...
idFile * idFileSystemLocal::GetResourceFile(const char *fileName, unsigned int cst_memFile) {
//#modified-fva; END
	...
	if ( GetResourceCacheEntry( fileName, rc ) ) {
		...
		if (file != NULL && (cst_memFile > 0 || rc.length <= resourceBufferAvailable || rc.length < 8 * 1024 * 1024)) {
		//#modified-fva; END
			byte *buf = NULL;
			//#modified-fva; BEGIN
			//if ( rc.length < resourceBufferAvailable ) {
			if (rc.length < resourceBufferAvailable && cst_memFile != 2) {
			//#modified-fva; END
...
idFile *idFileSystemLocal::OpenFileReadFlags( const char *relativePath, int searchFlags, bool allowCopyFiles, const char* gamedir ) {
	...
	if ( resourceFiles.Num() > 0 && fs_resourceLoadPriority.GetInteger() ==  1 ) {
		...
		unsigned int cst_memFile = 0;
		if (searchFlags & FSFLAG_RETURN_FILE_MEM) {
			if (searchFlags & FSFLAG_CST_ENSURE_FILE_MEM) {
				cst_memFile = 2;
			} else {
				cst_memFile = 1;
			}
		}
		idFile * rf = GetResourceFile(relativePath, cst_memFile);
		//#modified-fva; END
		if ( rf != NULL ) {
			return rf;
		}
...
//#modified-fva; BEGIN
idFile *idFileSystemLocal::CstOpenFileRead_FileMemory(const char *relativePath, bool allowCopyFiles, const char* gamedir) {
	return OpenFileReadFlags(relativePath, FSFLAG_SEARCH_DIRS | FSFLAG_RETURN_FILE_MEM | FSFLAG_CST_ENSURE_FILE_MEM, allowCopyFiles, gamedir);
}
//#modified-fva; END
