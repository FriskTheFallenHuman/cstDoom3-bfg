...
public:
	...
	//#modified-fva; BEGIN
	void					CstToLowerUTF8_Partial();
	void					CstToUpperUTF8_Partial();
	static char *			CstToLowerUTF8_Partial(char * s);
	static char *			CstToUpperUTF8_Partial(char * s);
	//#modified-fva; END
...
//#modified-fva; BEGIN
ID_INLINE void idStr::CstToLowerUTF8_Partial() {
	// handle only chars within the regular ascii portion
	for (int i = 0; data[i]; ++i) {
...
ID_INLINE void idStr::CstToUpperUTF8_Partial() {
	// handle only chars within the regular ascii portion
	for (int i = 0; data[i]; ++i) {
		if (data[i] <= 'z' && data[i] >= 'a') {
			data[i] -= ('a' - 'A');
...
ID_INLINE char * idStr::CstToLowerUTF8_Partial(char * s) {
	// handle only chars within the regular ascii portion
	for (int i = 0; s[i]; ++i) {
		if (s[i] <= 'Z' && s[i] >= 'A') {
			s[i] += ('a' - 'A');
...
ID_INLINE char * idStr::CstToUpperUTF8_Partial(char * s) {
	// handle only chars within the regular ascii portion
	for (int i = 0; s[i]; ++i) {
		if (s[i] <= 'z' && s[i] >= 'a') {
			s[i] -= ('a' - 'A');
