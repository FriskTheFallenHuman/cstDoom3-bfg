...
//#modified-fva; BEGIN
void idSoundWorldLocal::CstSetPauseFade() {
	pauseFade.SetVolume(DB_SILENCE);
	pauseFade.Fade(0.0f, s_unpauseFadeInTime.GetInteger(), GetSoundTime());
}
//#modified-fva; END
...
void idSoundWorldLocal::ReadFromSaveGame( idFile * savefile ) {
	...
	//#modified-fva; BEGIN
	savefile->ReadBool(cstMuted); // cstMuted must be restored after the ClearAllSoundEmitters call above
	//#modified-fva; END
}
