...
void idSoundEmitterLocal::Update( int currentTime ) {
	...
	if (soundSystemLocal.muted || soundWorld != soundSystemLocal.currentSoundWorld || soundWorld->cstMuted) {
	//#modified-fva; END
		return;
	}
	...
	bool cstHasGlobalChannel = false;
	//#modified-fva; END
	...
	if ( emitterId != soundWorld->listener.id ) {
		...
		//#modified-fva; BEGIN
		// moved from below (with changes)
		if (!cstHasGlobalChannel && directDistance >= maxDistance) {
