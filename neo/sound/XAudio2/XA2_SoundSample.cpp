...
void idSoundSample_XAudio2::LoadResource() {
	...
	for ( int i = 0; i < 2; i++ ) {
		...
		if ((i == 0) && !sampleName.Replace("/vo/", va("/vo/%s/", common->CstGetLatchedLanguage().c_str()))) {
		//#modified-fva; END
			i++;
		}
...
#if CST_LIBBINKDEC
		if (!loaded) {
			sampleName.SetFileExtension(".bik");
			loaded = CstLoadBink(sampleName);
			if (loaded) {
				return; // <-- watch out
			}
		}
...
//#modified-fva; BEGIN
#if CST_LIBBINKDEC
bool idSoundSample_XAudio2::CstLoadBink(const idStr & filename) {
	struct ExitGuard {
		ExitGuard(BinkHandle *& _pBinkHandle, idFile_Memory *& _iFile) : pBinkHandle(_pBinkHandle), iFile(_iFile) {}
		~ExitGuard() {
			if (pBinkHandle) {
	...
	idFile_Memory * iFile = (idFile_Memory *)fileSystem->CstOpenFileRead_FileMemory(filename.c_str());
	if (!iFile) {
		return false;
	}

	BinkHandle * pBinkHandle = NULL;
	if (Bink_Init((const uint8_t *)iFile->GetDataPtr(), iFile->Length(), &pBinkHandle) < 1) {
		fileSystem->CloseFile(iFile);
		return false;
	}
	ExitGuard exitGuard(pBinkHandle, iFile);

	uint32_t numAudioTracks;
	if (Bink_GetNumAudioTracks(pBinkHandle, &numAudioTracks) < 1 || numAudioTracks != 1) { // support 1 track only
		return false;
	}

	AudioInfo aInfo;
	if (Bink_GetAudioTrackDetails(pBinkHandle, 0, &aInfo) < 1 || (aInfo.nChannels != 1 && aInfo.nChannels != 2)) { // support mono or stereo only
		return false;
	}

	uint32_t numFrames;
	if (Bink_GetNumFrames(pBinkHandle, &numFrames) < 1 || numFrames == 0) {
		return false;
	}

	const uint32_t idealBufferSamples = (aInfo.idealBufferSize + 1) >> 1;

	idList<int16_t> audioBuffer(128 * 1024 * aInfo.nChannels); // granularity = 128k samples per channel = 256k bytes per channel
	uint32_t totalSamplesRead = 0;
	uint32_t currentFrame = 0;
	if (Bink_GotoFrame(pBinkHandle, 0) < 1) {
		return false;
	}
	while (currentFrame < numFrames - 1) { // -1 because GetNextFrame updates currentFrame with the frame number just decoded
		if (Bink_GetNextFrame_AudioOnly(pBinkHandle, &currentFrame) < 1) {
			return false;
		}
		audioBuffer.AssureSize(totalSamplesRead + idealBufferSamples);
		uint32_t bytesRead;
		if (Bink_GetAudioData(pBinkHandle, 0, audioBuffer.Ptr() + totalSamplesRead, &bytesRead) < 1) {
			return false;
		}
		if ((bytesRead & 0x1) != 0x0) {
			// it is assumed 16 bits per sample, so bytesRead must be even
			return false;
		}
		totalSamplesRead += bytesRead >> 1;
	}

	int64 fileTimestamp = iFile->Timestamp();
	...
	// ------------
	idStr ampFileName = filename;
	ampFileName.SetFileExtension(".amp");
	LoadAmplitude(ampFileName);
	...
	memset(&format, 0, sizeof(format));
	format.basic.formatTag = idWaveFile::FORMAT_PCM;
	format.basic.numChannels = (uint16)aInfo.nChannels;
	format.basic.samplesPerSec = aInfo.sampleRate;
	format.basic.avgBytesPerSec = aInfo.sampleRate * ((bitsPerSample * aInfo.nChannels) >> 3);
	format.basic.blockSize = (uint16)((aInfo.nChannels * bitsPerSample) >> 3);
	format.basic.bitsPerSample = (uint16)bitsPerSample;

	totalBufferSize = (int)(totalSamplesRead << 1);
	...
	buffers[0].numSamples = playLength;
	buffers[0].buffer = AllocBuffer(totalBufferSize, GetName());
	memcpy(buffers[0].buffer, audioBuffer.Ptr(), totalBufferSize);

	// see idSoundSample_XAudio2::LoadWav
	buffers[0].buffer = GPU_CONVERT_CPU_TO_CPU_CACHED_READONLY_ADDRESS(buffers[0].buffer);

	return true;
}
#endif // CST_LIBBINKDEC
//#modified-fva; END
