...
idForce_Grab::idForce_Grab() {
	damping			= 0.5f;
	//#modified-fva; BEGIN
	cstAdjustedLinearDamping = -1.0f;
	cstAdjustedAngularDamping = -1.0f;
	//#modified-fva; END
	physics			= NULL;
	id				= 0;
}
...
void idForce_Grab::Evaluate( int time ) {
	...
	//#modified-fva; BEGIN
	if (cstAdjustedLinearDamping < 0.0f) {
		cstAdjustedLinearDamping = idMath::Pow(damping, 60.0f / com_engineHz_latched);
	}
	if (cstAdjustedAngularDamping < 0.0f) {
		cstAdjustedAngularDamping = idMath::Pow(0.99999f, 60.0f / com_engineHz_latched);
	}
	//#modified-fva; END
	...
	if ( distanceToGoal < 196.f ) {
		...
		physics->SetLinearVelocity(v * cstAdjustedLinearDamping, id);
		//#modified-fva; END
	}
	if ( distanceToGoal < 16.f ) {
		...
		if ( v.LengthSqr() > Square(8) ) {
			...
			physics->SetAngularVelocity(v * cstAdjustedAngularDamping, id);
			//#modified-fva; END
		}
	}
}
