...
void idCameraView::GetViewParms( renderView_t *view ) {
	...
	gameLocal.CalcFov(fov, view->fov_x, view->fov_y, true);
	//#modified-fva; END
}
...
void idCameraAnim::LoadAnim() {
	...
	//#modified-fva; BEGIN
	if (filename.Icmp("models/md5/cinematics/marscity/window_camera.md5camera") == 0) {
		// exception for the monitor camera in the first cinematic of mars city
		cstOldStyle = false;
	} else {
		cstOldStyle = true;
	}
...
void idCameraAnim::GetViewParms( renderView_t *view ) {
	...
	gameLocal.CalcFov(view->fov_x, view->fov_x, view->fov_y, cstOldStyle);
	//#modified-fva; END
