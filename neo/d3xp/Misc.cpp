...
idStaticEntity::idStaticEntity() {
	...
	runGui = false;
	//#modified-fva; BEGIN
	cstCinematicEntity = false;
	cstUpdateCinematics = false;
	//#modified-fva; END
}
...
void idStaticEntity::Save( idSaveGame *savefile ) const {
	...
	//#modified-fva; BEGIN
	savefile->WriteBool(cstCinematicEntity);
	savefile->WriteBool(cstUpdateCinematics);
	//#modified-fva; END
}
...
void idStaticEntity::Restore( idRestoreGame *savefile ) {
	...
	//#modified-fva; BEGIN
	savefile->ReadBool(cstCinematicEntity);
	savefile->ReadBool(cstUpdateCinematics);
	if (cstUpdateCinematics && renderEntity.gui[0]) {
		renderEntity.gui[0]->CstRequestCinematicsUpdate(-1); // a negative time is replaced by the the actual time in idGameLocal::InitFromSaveGame
		if (renderEntity.gui[1]) {
			renderEntity.gui[1]->CstRequestCinematicsUpdate(-1); // as above
		}
		if (renderEntity.gui[2]) {
			renderEntity.gui[2]->CstRequestCinematicsUpdate(-1); // as above
		}
	}
	//#modified-fva; END
}
...
void idStaticEntity::Spawn() {
	...
	//#modified-fva; BEGIN
	struct local {
		static void CstCheckHealthStation(idStaticEntity &ent) {
			if (common->IsMultiplayer() || !cst_oldHealthStation.GetBool()) {
				return;
			}
			if (idStr::Icmp(ent.spawnArgs.GetString("gui", ""), "guis/controlpanels/healthstation.gui") != 0) {
				return;
			}
			const idDict *def = gameLocal.FindEntityDefDict("cst_old_healthstation", false);
			if (!def) {
				return;
			}
			idStr key = idStr(gameLocal.GetMapName()) + '/' + ent.name;
			int value = def->GetInt(key.c_str(), -1);
			if (value < 0) {
				return;
			}
			ent.spawnArgs.SetInt("gui_parm1", value);
			if (ent.GetRenderEntity() && ent.GetRenderEntity()->gui[0]) {
				ent.GetRenderEntity()->gui[0]->SetStateInt("gui_parm1", value);
				ent.GetRenderEntity()->gui[0]->StateChanged(gameLocal.fast.time);
			}
		}
	};
	local::CstCheckHealthStation(*this);
	//#modified-fva; END
	...
	*/
	if (!common->IsMultiplayer() && renderEntity.gui[0]) {
		if (renderEntity.gui[0]->CstHasCinematics() ||
			renderEntity.gui[1] && renderEntity.gui[1]->CstHasCinematics() ||
			renderEntity.gui[2] && renderEntity.gui[2]->CstHasCinematics()) {
			cstUpdateCinematics = true;
		}
	}
	cstCinematicEntity = cinematic;
	if (cstUpdateCinematics) {
		cinematic = true;
	}
	if (runGui || cstUpdateCinematics) {
		BecomeActive(TH_THINK);
	}
	//#modified-fva; END
}
...
void idStaticEntity::Think() {
	...
	if ( thinkFlags & TH_THINK ) {
		...
		*/
		if (cstUpdateCinematics && renderEntity.gui[0]) {
			renderEntity.gui[0]->CstRequestCinematicsUpdate(gameLocal.fast.time);
			if (renderEntity.gui[1]) {
				renderEntity.gui[1]->CstRequestCinematicsUpdate(gameLocal.fast.time);
			}
			if (renderEntity.gui[2]) {
				renderEntity.gui[2]->CstRequestCinematicsUpdate(gameLocal.fast.time);
			}
		}
		if (gameLocal.inCinematic && !cstCinematicEntity) {
			return; // <-- watch out
		}
		if (runGui && renderEntity.gui[0]) {
			renderEntity.gui[0]->StateChanged(gameLocal.fast.time, true);
			if (renderEntity.gui[1]) {
				renderEntity.gui[1]->StateChanged(gameLocal.fast.time, true);
			}
			if (renderEntity.gui[2]) {
				renderEntity.gui[2]->StateChanged(gameLocal.fast.time, true);
			}
		}
		//#modified-fva; END
		if ( fadeEnd > 0 ) {
