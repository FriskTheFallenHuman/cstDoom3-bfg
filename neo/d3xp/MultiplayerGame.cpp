...
void idMultiplayerGame::PlayGlobalSound( int toPlayerNum, snd_evt_t evt, const char *shader ) {
	//#modified-fva; BEGIN
	int cstLocalClientNum = gameLocal.GetLocalClientNum();
	//#modified-fva; END

	//#modified-fva; BEGIN
	//if ( toPlayerNum < 0 || toPlayerNum == gameLocal.GetLocalClientNum() ) {
	if (cstLocalClientNum >= 0 && (toPlayerNum < 0 || toPlayerNum == cstLocalClientNum)) {
	//#modified-fva; END
		if ( shader ) {
			if ( gameSoundWorld ) {
	...
	if (common->IsServer() && (toPlayerNum < 0 || toPlayerNum != cstLocalClientNum)) {
	//#modified-fva; END
		idBitMsg outMsg;
		byte msgBuf[1024];
		outMsg.InitWrite( msgBuf, sizeof( msgBuf ) );
