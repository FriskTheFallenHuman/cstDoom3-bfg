...
//#modified-fva; BEGIN
struct cstAchievementDetails_t {
	int		achievementID;	// achievement's id number
	idStr	name;			// achievement's title; empty if hidden or unknown
	idStr	description;	// achievement's description; empty if hidden or unknown
	bool	gameDependent;	// true if the achievement is game dependent (must be unlocked in a single playthrough)
	int		state;			// 0 if locked, 1 if unlocked, -1 if unknown
	int		count;			// current count; -1 if unknown
	int		required;		// required count to unlock
};
//#modified-fva; END
...
public:
	...
	void		EventCompletesAchievement(const achievement_t eventId, bool cstNotify = true);
	//#modified-fva; END
	...
	//#modified-fva; BEGIN
	static void CstGetAchievementsDetails(idList<cstAchievementDetails_t> & detailsList, bool getLocalizationKeys);
	//#modified-fva; END
