...
//#modified-fva; BEGIN
const idEventDef EV_Player_CstDamage("cstDamage", "s");
const idEventDef EV_Player_CstGetAwDbsD3("cstGetAwDbsD3", NULL, 'd');
const idEventDef EV_Player_CstGetAwGrabberD3("cstGetAwGrabberD3", NULL, 'd');
const idEventDef EV_Player_CstGetAwChainsawRoE("cstGetAwChainsawRoE", NULL, 'd');
const idEventDef EV_Player_CstGetAwChainsawLM("cstGetAwChainsawLM", NULL, 'd');
//#modified-fva; END
...
CLASS_DECLARATION( idActor, idPlayer )
	...
	//#modified-fva; BEGIN
	EVENT(EV_Player_CstDamage, idPlayer::Event_CstDamage)
	EVENT(EV_Player_CstGetAwDbsD3, idPlayer::Event_CstGetAwDbsD3)
	EVENT(EV_Player_CstGetAwGrabberD3, idPlayer::Event_CstGetAwGrabberD3)
	EVENT(EV_Player_CstGetAwChainsawRoE, idPlayer::Event_CstGetAwChainsawRoE)
	EVENT(EV_Player_CstGetAwChainsawLM, idPlayer::Event_CstGetAwChainsawLM)
	//#modified-fva; END
END_CLASS
...
//#modified-fva; BEGIN
const char CST_OLD_PM_CROUCH_SPEED[] = "80";
const char CST_OLD_PM_WALK_SPEED_SP[] = "140";
const char CST_OLD_PM_WALK_SPEED_MP[] = "160";
const char CST_OLD_PM_RUN_SPEED_SP[] = "220";
const char CST_OLD_PM_RUN_SPEED_MP[] = "270";
//#modified-fva; END
	...
	//	owner->GivePowerUp( HASTE, SEC2MS( atof( value ) ) );
		...
		for( pos = value; pos != NULL; pos = end ) {
			...
			if ( !gameLocal.world->spawnArgs.GetBool( "no_Weapons" ) || ( weaponName == "weapon_fists" ) || ( weaponName == "weapon_soulcube" ) ) {
				...
				//#modified-fva; BEGIN
				else if (!common->IsMultiplayer() && cst_takeNoAmmoWeapons.GetBool() && (weapons & (1 << i)) && weaponDecl && !weaponDecl->dict.GetInt("ammoRequired")) {
					tookWeapon = true;
				}
				//#modified-fva; END
			}
			//#modified-fva; BEGIN
			if (!common->IsMultiplayer() && tookWeapon) {
				gameExpansionType_t expansion = owner->GetExpansionType();
				if (expansion == GAME_BASE) {
					if (weaponName == "weapon_shotgun_double") {
						owner->cstAwDoubleBarrelShotgunD3 = 1;
					} else if (weaponName == "weapon_grabber") {
						owner->cstAwGrabberD3 = 1;
					}
				} else if (expansion == GAME_D3XP) {
					if (weaponName == "weapon_chainsaw") {
						owner->cstAwChainsawRoE = 1;
					}
				} else if (expansion == GAME_D3LE) {
					if (weaponName == "weapon_chainsaw") {
						owner->cstAwChainsawLM = 1;
					}
				}
			}
			//#modified-fva; END
...
idPlayer::idPlayer():
	...
	weapon_chainsaw			= -1;
	//#modified-fva; BEGIN
	cst_weapon_grabber			= -1;
	cst_weapon_shotgun			= -1;
	cst_weapon_shotgun_double	= -1;
	cst_weapon_handgrenade		= -1;
	//#modified-fva; END
	weapon_bloodstone		= -1;
...
void idPlayer::Init() {
	...
	//#modified-fva; BEGIN
	cst_weapon_grabber			= SlotForWeapon("weapon_grabber");
	cst_weapon_shotgun			= SlotForWeapon("weapon_shotgun");
	cst_weapon_shotgun_double	= SlotForWeapon("weapon_shotgun_double");
	cst_weapon_handgrenade		= SlotForWeapon("weapon_handgrenade");
	//#modified-fva; END
	...
	if ( !common->IsMultiplayer() || common->IsServer() ) {
		...
		//#modified-fva; BEGIN
		bool useOldPlayerSpeed = common->IsMultiplayer() ? gameLocal.cstSiOldPlayerSpeedMP : cst_oldPlayerSpeedSP.GetBool();
		if (useOldPlayerSpeed) {
			cvarSystem->SetCVarString("pm_crouchspeed", CST_OLD_PM_CROUCH_SPEED);
			if (common->IsMultiplayer()) {
				cvarSystem->SetCVarString("pm_walkspeed", CST_OLD_PM_WALK_SPEED_MP);
				cvarSystem->SetCVarString("pm_runspeed", CST_OLD_PM_RUN_SPEED_MP);
			} else {
				cvarSystem->SetCVarString("pm_walkspeed", CST_OLD_PM_WALK_SPEED_SP);
				cvarSystem->SetCVarString("pm_runspeed", CST_OLD_PM_RUN_SPEED_SP);
			}
		}
		//#modified-fva; END
	}
...
void idPlayer::Save( idSaveGame *savefile ) const {
	...
	//#modified-fva; BEGIN
	savefile->WriteInt(cst_weapon_grabber);
	savefile->WriteInt(cst_weapon_shotgun);
	savefile->WriteInt(cst_weapon_shotgun_double);
	savefile->WriteInt(cst_weapon_handgrenade);
	//#modified-fva; END
	...
	//#modified-fva; BEGIN
	savefile->WriteInt(cstAwDoubleBarrelShotgunD3);
	savefile->WriteInt(cstAwGrabberD3);
	savefile->WriteInt(cstAwChainsawRoE);
	savefile->WriteInt(cstAwChainsawLM);
	//#modified-fva; END
	...
	//#modified-fva; BEGIN
	savefile->WriteBool(cstPlayHealthPulseSound);
	//#modified-fva; END
	...
	//#modified-fva; BEGIN
	savefile->WriteString(cstLastTip.title);
	savefile->WriteString(cstLastTip.tip);
	savefile->WriteBool(cstLastTip.autoHide);
	//#modified-fva; END
	savefile->WriteBool( objectiveUp );
	//#modified-fva; BEGIN
	savefile->WriteBool(cstObjectiveCompleteUp);
	savefile->WriteString(hud ? hud->CstGetObjectiveCompleteTitle().c_str() : "");
	savefile->WriteBool(cstNewVideoUp);
	savefile->WriteBool(cstNewPDAUp);
	savefile->WriteBool(hudManager ? hudManager->CstIsRadioActive() : false);
	//#modified-fva; END
...
void idPlayer::Restore( idRestoreGame *savefile ) {
	...
	//#modified-fva; BEGIN
	savefile->ReadInt(cst_weapon_grabber);
	savefile->ReadInt(cst_weapon_shotgun);
	savefile->ReadInt(cst_weapon_shotgun_double);
	savefile->ReadInt(cst_weapon_handgrenade);
	//#modified-fva; END
	...
	//#modified-fva; BEGIN
	savefile->ReadInt(cstAwDoubleBarrelShotgunD3);
	savefile->ReadInt(cstAwGrabberD3);
	savefile->ReadInt(cstAwChainsawRoE);
	savefile->ReadInt(cstAwChainsawLM);
	//#modified-fva; END
	...
	//#modified-fva; BEGIN
	savefile->ReadBool(cstPlayHealthPulseSound);
	//#modified-fva; END
	...
	//#modified-fva; BEGIN
	savefile->ReadString(cstLastTip.title);
	savefile->ReadString(cstLastTip.tip);
	savefile->ReadBool(cstLastTip.autoHide);
	//#modified-fva; END
	savefile->ReadBool( objectiveUp );
	//#modified-fva; BEGIN
	savefile->ReadBool(cstObjectiveCompleteUp);
	idStr cstObjectiveCompleteTitle;
	savefile->ReadString(cstObjectiveCompleteTitle);
	savefile->ReadBool(cstNewVideoUp);
	savefile->ReadBool(cstNewPDAUp);
	bool cstIsRadioActive;
	savefile->ReadBool(cstIsRadioActive);
	//#modified-fva; END
	...
	if (hud) {
		...
		if (cstObjectiveCompleteUp && cstObjectiveCompleteTitle.Length() > 0) {
			hud->SetupObjectiveComplete(cstObjectiveCompleteTitle);
		}
		if (hudManager && cstIsRadioActive) {
			hudManager->SetRadioMessage(true);
		}
		hud->CstUpdateOnRestore(true);
	}
	//#modified-fva; END
...
void idPlayer::SavePersistantInfo() {
	...
	//#modified-fva; BEGIN
	playerInfo.SetInt("cstAwDoubleBarrelShotgunD3", cstAwDoubleBarrelShotgunD3);
	playerInfo.SetInt("cstAwGrabberD3", cstAwGrabberD3);
	playerInfo.SetInt("cstAwChainsawRoE", cstAwChainsawRoE);
	playerInfo.SetInt("cstAwChainsawLM", cstAwChainsawLM);

	cstHeadlamp.SavePersistantInfo(playerInfo);
	//#modified-fva; END
}
...
void idPlayer::RestorePersistantInfo() {
	...
	//#modified-fva; BEGIN
	if (!common->IsMultiplayer()) {
		cstAwDoubleBarrelShotgunD3 = spawnArgs.GetInt("cstAwDoubleBarrelShotgunD3", -1);
		cstAwGrabberD3 = spawnArgs.GetInt("cstAwGrabberD3", -1);
		cstAwChainsawRoE = spawnArgs.GetInt("cstAwChainsawRoE", -1);
		cstAwChainsawLM = spawnArgs.GetInt("cstAwChainsawLM", -1);
		gameExpansionType_t expansion = GetExpansionType();
		if (expansion == GAME_BASE) {
			if (cstAwDoubleBarrelShotgunD3 < 0) {
				cstAwDoubleBarrelShotgunD3 = cst_awDoubleBarrelShotgunD3.GetBool() ? 1 : 0;
			}
			if (cstAwGrabberD3 < 0) {
				cstAwGrabberD3 = cst_awGrabberD3.GetBool() ? 1 : 0;
			}
		} else if (expansion == GAME_D3XP) {
			if (cstAwChainsawRoE < 0) {
				cstAwChainsawRoE = cst_awChainsawRoE.GetBool() ? 1 : 0;
			}
		} else if (expansion == GAME_D3LE) {
			if (cstAwChainsawLM < 0) {
				cstAwChainsawLM = cst_awChainsawLM.GetBool() ? 1 : 0;
			}
		}
	}

	cstHeadlamp.RestorePersistantInfo(spawnArgs);
	//#modified-fva; END
}
...
void idPlayer::UpdateHudStats( idMenuHandler_HUD * _hudManager ) {
	...
	if ( _hudManager && _hudManager->GetHud() ) {
		...
		//#modified-fva; BEGIN
		hud->CstUpdateZoomIndicator(cst_hudZoom.GetBool() && (usercmd.buttons & BUTTON_ZOOM));
		hud->CstUpdateCrouchIndicator(cst_hudCrouch.GetBool() && (usercmd.buttons & BUTTON_CROUCH));
		hud->CstUpdateRunIndicator(cst_hudRun.GetBool() && (usercmd.buttons & BUTTON_RUN));
		//#modified-fva; END
...
void idPlayer::DrawHUD( idMenuHandler_HUD * _hudManager ) {
	...
	if (!weapon.GetEntity() || influenceActive != INFLUENCE_NONE || privateCameraView || gameLocal.GetCamera() || !g_showHud.GetBool() || gameLocal.Cst_Shell_IsGameComplete()) {
	//#modified-fva; END
		return;
	}
...
bool idPlayer::GiveItem( idItem *item, unsigned int giveFlags ) {
	...
	*/
	{
		bool isHandFlashlightAllowed = common->IsMultiplayer() ? gameLocal.cstSiAllowHandFlashlightMP : true;
		if (!isHandFlashlightAllowed && idStr::FindText(item->GetName(), "weapon_flashlight") > -1) {
			return false;
		}
	}
	//#modified-fva; END
...
void idPlayer::SelectWeapon( int num, bool force ) {
	...
	if ( force || ( inventory.weapons & ( 1 << num ) ) ) {
		...
		bool cstSwitchToPrevious = false;
		if ((previousWeapon >= 0) && (idealWeapon == num)) {
			if (spawnArgs.GetBool(va("weapon%d_toggle", num))) {
				cstSwitchToPrevious = true;
			} else if (idealWeapon == cst_weapon_handgrenade && (!common->IsMultiplayer() || gameLocal.cstSiAllowGrenadesToggleMP)) {
				idLobbyBase &lobby = session->GetActingGameStateLobbyBase();
				lobbyUserID_t &lobbyUserID = gameLocal.lobbyUserIDs[entityNumber];
				cstSwitchToPrevious = lobby.CstGetLobbyUserGrenadesToggle(lobbyUserID);
			}
		}
		if (cstSwitchToPrevious) {
		//#modified-fva; END
...
void idPlayer::StealWeapon( idPlayer *player ) {
	...
	*/
	...
	if (ammoavailable != -1) {
		...
		int cstMaxAmmo = inventory.MaxAmmoForAmmoClass(this, idWeapon::GetAmmoNameForNum(ammo_i));
		int cstAmmoCount = ammoavailable + inclip + inventory.GetInventoryAmmoForType(ammo_i);
		if (cstMaxAmmo > 0 && cstAmmoCount > cstMaxAmmo) {
			cstAmmoCount = cstMaxAmmo;
		}
		inventory.SetInventoryAmmoForType(ammo_i, cstAmmoCount);
	}
	idealWeapon = newweap;
	//#modified-fva; END
}
	...
	if (  idealWeapon != currentWeapon &&  idealWeapon.Get() < MAX_WEAPONS ) {
		if ( weaponCatchup ) {
			...
			if ( weapon.GetEntity()->IsHolstered() ) {
				...
				bool cstUpdatePrevious = true;
				if (currentWeapon == weapon_pda || spawnArgs.GetBool(va("weapon%d_toggle", currentWeapon))) {
					cstUpdatePrevious = false;
				} else if (currentWeapon == cst_weapon_handgrenade && (!common->IsMultiplayer() || gameLocal.cstSiAllowGrenadesToggleMP)) {
					idLobbyBase &lobby = session->GetActingGameStateLobbyBase();
					lobbyUserID_t &lobbyUserID = gameLocal.lobbyUserIDs[entityNumber];
					cstUpdatePrevious = !lobby.CstGetLobbyUserGrenadesToggle(lobbyUserID);
				}
				if (cstUpdatePrevious) {
				//#modified-fva; END
...
void idPlayer::UpdateFlashlight() {
	...
	//#modified-fva; BEGIN
	// see the show/hide logic near the end of this function
	/*
	...
	*/
	...
	//#modified-fva; BEGIN
	if (cstArmorFlashlightHideModel != cst_armorFlashlightHideModel.GetInteger()) {
		cstArmorFlashlightHideModel = cst_armorFlashlightHideModel.GetInteger();
		flashlight.GetEntity()->UpdateSkin();
		if (cstArmorFlashlightHideModel == 1) {
			worldModel->GetRenderEntity()->noShadow = true;
		} else {
			worldModel->GetRenderEntity()->noShadow = false;
		}
	}
	//#modified-fva; END
	...
	if (gameLocal.world->spawnArgs.GetBool("no_Weapons") || gameLocal.inCinematic || spectating || fl.hidden || (common->IsMultiplayer() && health <= 0)) {
	//#modified-fva; END
		worldModel->Hide();
	} else {
		worldModel->Show();
	...
	if ( pdaVideoMat ) {
		...
		for ( int i = 0; i < c; i++ ) {
			...
			if ( stage != NULL && stage->texture.cinematic ) {
				...
				stage->texture.cinematic->CstResetTime_Sys(Sys_Milliseconds());
				//#modified-fva; END
			}
		}
...
void idPlayer::UpdatePowerupHud() {
	...
	if (lastHudPowerup != hudPowerup) {
		if (lastHudPowerup == ENVIROTIME && hud) {
			hud->CstUpdateEnviro(false, 0, 0); // turn off
		}
		lastHudPowerup = hudPowerup;
	}
	if (hudPowerup == ENVIROTIME && hud) {
		...
		hud->CstUpdateEnviro(true, bar, value);
	}
}
//#modified-fva; END
...
void idPlayer::PerformImpulse( int impulse ) {
	...
	//#modified-fva; BEGIN
	if (impulse == IMPULSE_62 || impulse == IMPULSE_63) {
		if (common->IsMultiplayer()) {
			return;
		}
		if (weapon.GetEntity() && weapon.GetEntity()->GetGrabberState() == 2) {
			int cstDropOnLayerChange;
			if (impulse == IMPULSE_62) {
				cstDropOnLayerChange = cst_grabberDropOnSwitchLayer.GetInteger();
			} else { // IMPULSE_63
				cstDropOnLayerChange = cst_grabberDropOnHoldLayer.GetInteger();
			}
			if (cstDropOnLayerChange == -1 || (cstDropOnLayerChange > 0 && cstDropOnLayerChange == usercmd.cstLayer)) {
				impulse = IMPULSE_13; // reload to drop
			} else {
				return;
			}
		} else {
	...
	switch( impulse ) {
		...
		case IMPULSE_16: {
			if( flashlight.IsValid() ) {
				...
				} else if (currentWeapon != weapon_flashlight && !spectating && weaponEnabled && !hiddenWeapon && !gameLocal.world->spawnArgs.GetBool("no_Weapons")) {
				//#modified-fva; END
					FlashlightOn();
				}
			}
		...
		case IMPULSE_32: {
			...
			if (isHandFlashlightAllowed) {
				if (currentWeapon != weapon_flashlight) {
					FlashlightOff(); // turn off the armor-mounted flashlight before selecting the handheld flashlight
				}
				SelectWeapon(16, false);
			}
			break;
		}
		//#modified-fva; END
...
void idPlayer::Move_Interpolated( float fraction ) {
	...
	if ( EyeHeight() != newEyeOffset ) {
		if ( spectating ) {
			...
			float crouchRate = cstCrouchRate.GetValue();
...
void idPlayer::Move() {
	...
	//#modified-fva; BEGIN
	bool cstWasOnGround = physicsObj.HasGroundContacts();
	//#modified-fva; END
	...
	//#modified-fva; BEGIN
	//CrashLand( oldOrigin, oldVelocity );
	if (!common->IsClient() || (IsLocallyControlled() && !cstWasOnGround)) {
		CrashLand(oldOrigin, oldVelocity);
	}
	//#modified-fva; END
}
...
void idPlayer::Think() {
	...
	//#modified-fva; from doom3 gpl; BEGIN
	if (gameLocal.inCinematic && gameLocal.skipCinematic) {
		return;
	}
	//#modified-fva; from doom3 gpl; END
	...
	if ( spectating ) {
		...
		UpdateWeapon();
		//#modified-fva; BEGIN
		weapon.GetEntity()->CstUpdateGrabber();
		//#modified-fva; END
	}
...
void idPlayer::Damage( idEntity *inflictor, idEntity *attacker, const idVec3 &dir,
	...
	if ( damageDef->dict.GetBool( "burn" ) ) {
		...
		if ( !armorSave && health > 0 ) {
			...
			if (entityNumber != gameLocal.GetLocalClientNum() || !cst_dfNoSoundAirGasp.GetBool()) {
				StartSound("snd_airGasp", SND_CHANNEL_ITEM, 0, false, NULL);
			}
			//#modified-fva; END
		}
...
void idPlayer::WriteToSnapshot( idBitMsg &msg ) const {
	...
	msg.WriteBits(cstPulseArmor, 1);
	//#modified-fva; END

	//#modified-fva; BEGIN
	cstHeadlamp.WriteToSnapshot(msg);
	//#modified-fva; END
}
...
void idPlayer::ReadFromSnapshot( const idBitMsg &msg ) {
	...
	//#modified-fva; BEGIN
	bool cstPulseArmor = msg.ReadBits(1) != 0;
	//#modified-fva; END

	//#modified-fva; BEGIN
	cstHeadlamp.ReadFromSnapshot(this, msg);
	//#modified-fva; END
	...
	*/
	...
	//#modified-fva; BEGIN
	if (cstPulseArmor && !snapshotStale) {
		// see idPlayer::Damage
		int cstBaseTime = gameLocal.fast.realClientTime + (gameLocal.fast.time - gameLocal.fast.previousTime);

		//if (gameLocal.time > lastArmorPulse + 200) {
		if (cstBaseTime > lastArmorPulse + 200) {
			StartSound("snd_hitArmor", SND_CHANNEL_ITEM, 0, false, NULL);
		}
		//lastArmorPulse = gameLocal.time;
		lastArmorPulse = cstBaseTime;
	}
	//#modified-fva; END
...
void idPlayer::ShowTip( const char *title, const char *tip, bool autoHide ) {
	...
	if ( hudManager ) {
		...
		//#modified-fva; BEGIN
		tipUp = true;
		cstLastTip.title = title;
		cstLastTip.tip = tip;
		cstLastTip.autoHide = autoHide;
		//#modified-fva; END
	}
	//#modified-fva; BEGIN
	//tipUp = true;
...
//#modified-fva; BEGIN
void idPlayer::CstShowLastTip() {
	if (hudManager && cstLastTip.title.Length() > 0 && cstLastTip.tip.Length() > 0) {
		hudManager->ShowTip(cstLastTip.title, cstLastTip.tip, cstLastTip.autoHide);
		tipUp = true;
	}
}
//#modified-fva; END
...
void idPlayer::HideObjective() {
	...
	*/
	if (!objectiveUp && !cstObjectiveCompleteUp) {
		return;
	}
	...
	if (hud) {
		if (objectiveUp) {
			hud->HideObjective(false);
			objectiveUp = false;
		} else if (cstObjectiveCompleteUp) {
			hud->HideObjective(true);
			cstObjectiveCompleteUp = false;
		}
	}
...
//#modified-fva; BEGIN
idPlayer::CstHeadlamp::CstHeadlamp() {
...
// ==========
void idPlayer::CstHeadlamp::Init(idPlayer *owner) {
	if (common->IsMultiplayer()) {
		lampOn = false;
	}
}
...
// ==========
void idPlayer::CstHeadlamp::Save(idSaveGame *savefile) const {
	savefile->WriteRenderLight(renderLight);
	savefile->WriteInt(lightDefHandle);
	savefile->WriteBool(lampOn);
}

// ==========
void idPlayer::CstHeadlamp::Restore(idRestoreGame *savefile) {
	savefile->ReadRenderLight(renderLight);
	savefile->ReadInt(lightDefHandle);
	savefile->ReadBool(lampOn);
...
// ==========
void idPlayer::CstHeadlamp::SavePersistantInfo(idDict &dict) {
	dict.SetBool("cstHeadlampOn", lampOn);
}

// ==========
void idPlayer::CstHeadlamp::RestorePersistantInfo(const idDict &dict) {
	if (!common->IsMultiplayer() && cst_headlampKeepState.GetBool()) {
		lampOn = dict.GetBool("cstHeadlampOn", "0");
	} else {
		lampOn = false;
	}
}
...
// ==========
void idPlayer::CstHeadlamp::WriteToSnapshot(idBitMsg &msg) const {
	msg.WriteBits(lampOn, 1);
}

// ==========
void idPlayer::CstHeadlamp::ReadFromSnapshot(idPlayer *owner, const idBitMsg &msg) {
	bool snapLampOn = msg.ReadBits(1) != 0;
...
// ==========
void idPlayer::CstHeadlamp::Toggle(idPlayer *owner) {
	if (!IsAllowed(owner)) {
		return;
	}
	if (cst_headlampSound.GetBool()) {
		owner->StartSound(lampOn ? "snd_cst_headlamp_off" : "snd_cst_headlamp_on", SND_CHANNEL_ANY, SSF_PRIVATE_SOUND, false, NULL);
	}
	lampOn = !lampOn;
}

// ==========
void idPlayer::CstHeadlamp::Update(idPlayer *owner) {
	if (!lampOn || !IsAllowed(owner)) {
		Remove();
		return;
	}
	if (lightDefHandle < 0) {
	...
	if (!pm_thirdPerson.GetBool()) {
		...
		if (owner->entityNumber == localClientNum) {
			firstPerson = true;
		} else if (localClientNum >= 0 && gameLocal.entities[localClientNum] && gameLocal.entities[localClientNum]->IsType(idPlayer::Type)) {
			idPlayer *localPlayer = static_cast<idPlayer *>(gameLocal.entities[localClientNum]);
			if (localPlayer->spectating && localPlayer->spectator == owner->entityNumber) {
				firstPerson = true;
			}
		}
	}
	if (firstPerson) {
		renderLight.origin = owner->firstPersonViewOrigin + owner->firstPersonViewAxis[2] * 3;
		renderLight.axis = owner->firstPersonViewAxis;
		extern idCVar g_weaponShadows;
		renderLight.noShadows = !g_weaponShadows.GetBool();
	} else {
		idVec3 origin;
		idMat3 dummyAxis;
		owner->GetJointWorldTransform(owner->headJoint, gameLocal.time, origin, dummyAxis);
	...
	if (!common->IsMultiplayer() && cst_headlampAlertMonsters.GetBool() && !owner->fl.notarget) {
		AlertMonsters(owner);
	}
}

// ==========
void idPlayer::CstHeadlamp::Create(idPlayer *owner) {
	const idDict *args = gameLocal.FindEntityDefDict("cst_player_headlamp", false);
	if (!args) {
		return;
	}
...
// ==========
void idPlayer::CstHeadlamp::Remove() {
	if (lightDefHandle >= 0) {
...
// ==========
bool idPlayer::CstHeadlamp::IsAllowed(idPlayer *owner) {
	if (common->IsMultiplayer() && !gameLocal.cstSiAllowHeadlampMP) {
		lampOn = false;
		return false;
	}
	if (owner->health <= 0
		...
		|| owner->enviroSuitLight.IsValid()
		|| owner->currentWeapon == owner->weapon_flashlight // handheld flashlight
		|| (owner->flashlight.IsValid() && owner->flashlight.GetEntity()->lightOn) // armor-mounted flashlight
		|| owner->currentWeapon == owner->weapon_pda) {
		return false;
	}
	return true;
}
...
// ==========
void idPlayer::CstHeadlamp::AlertMonsters(idPlayer *owner) {
	// code below is adapted from idWeapon::AlertMonsters

	trace_t	tr;
	idEntity *ent;
	//idVec3 end = muzzleFlash.origin + muzzleFlash.axis * muzzleFlash.target;
	idVec3 end = renderLight.origin + renderLight.axis * renderLight.target;
	...
	gameLocal.clip.TracePoint(tr, renderLight.origin, end, CONTENTS_OPAQUE | MASK_SHOT_RENDERMODEL | CONTENTS_FLASHLIGHT_TRIGGER, owner);
	if (g_debugWeapon.GetBool()) {
		//gameRenderWorld->DebugLine(colorYellow, muzzleFlash.origin, end, 0);
		gameRenderWorld->DebugLine(colorYellow, renderLight.origin, end, 0);
		//gameRenderWorld->DebugArrow(colorGreen, muzzleFlash.origin, tr.endpos, 2, 0);
		gameRenderWorld->DebugArrow(colorGreen, renderLight.origin, tr.endpos, 2, 0);
	}
	...
	end += renderLight.axis * renderLight.right * idMath::Sin16(MS2SEC(gameLocal.time) * 31.34f);
	//end += muzzleFlash.axis * muzzleFlash.up * idMath::Sin16(MS2SEC(gameLocal.time) * 12.17f);
	end += renderLight.axis * renderLight.up * idMath::Sin16(MS2SEC(gameLocal.time) * 12.17f);
	//gameLocal.clip.TracePoint(tr, muzzleFlash.origin, end, CONTENTS_OPAQUE | MASK_SHOT_RENDERMODEL | CONTENTS_FLASHLIGHT_TRIGGER, owner);
	gameLocal.clip.TracePoint(tr, renderLight.origin, end, CONTENTS_OPAQUE | MASK_SHOT_RENDERMODEL | CONTENTS_FLASHLIGHT_TRIGGER, owner);
	if (g_debugWeapon.GetBool()) {
		//gameRenderWorld->DebugLine(colorYellow, muzzleFlash.origin, end, 0);
...
// ==========
idPlayer::CstCrouchRate::CstCrouchRate() {
	Reset();
}

// ==========
void idPlayer::CstCrouchRate::Reset() {
	baseRate = 0.0f;
	adjustedRate = 0.0f;
}

// ==========
float idPlayer::CstCrouchRate::GetValue() {
...
// ==========
void idPlayer::CstShowAccessCode(idUserInterface *focusUI) {
	if (common->IsMultiplayer() || !hud || !cst_showAccessCodes.GetBool()) {
		return;
	}
	const idDict *def = gameLocal.FindEntityDefDict("cst_access_codes", false);
	if (!def) {
		return;
	}
	idStr guiName = focusUI->GetStateString("name", ""); // non-localized name
	if (guiName.IsEmpty()) {
		return;
	}
	idStr value = def->GetString(("_" + guiName).c_str(), "");
	if (value.IsEmpty()) {
		return;
	}
	if (value == "gui_parms") {
		idStr guiParm1 = focusUI->GetStateString("gui_parm1", "");
		idStr guiParm2 = focusUI->GetStateString("gui_parm2", "");
		idStr guiParm3 = focusUI->GetStateString("gui_parm3", "");
		if (!guiParm1.IsEmpty() && !guiParm2.IsEmpty() && !guiParm3.IsEmpty()) {
			idStr code = guiParm1 + guiParm2 + guiParm3;
			if (code == "zzz") {
				return; // fix for doorkeypad.gui in alphalabs4
			}
			hud->CstShowAccessCode(code);
		} else {
			return;
		}
	} else {
		hud->CstShowAccessCode(value);
	}
}

// ==========
void idPlayer::CstHideAccessCode() {
	if (!common->IsMultiplayer() && hud) {
		hud->CstHideAccessCode();
	}
}

// ==========
void idPlayer::Event_CstDamage(const char *damageDef) {
...
// ==========
void idPlayer::Event_CstGetAwDbsD3() {
	idThread::ReturnInt(cstAwDoubleBarrelShotgunD3);
}

// ==========
void idPlayer::Event_CstGetAwGrabberD3() {
	idThread::ReturnInt(cstAwGrabberD3);
}

// ==========
void idPlayer::Event_CstGetAwChainsawRoE() {
	idThread::ReturnInt(cstAwChainsawRoE);
}

// ==========
void idPlayer::Event_CstGetAwChainsawLM() {
	idThread::ReturnInt(cstAwChainsawLM);
}
//#modified-fva; END
