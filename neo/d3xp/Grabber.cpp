	...
	if ( dragEnt.IsValid() ) {
		...
		if ( !ent->IsType( idProjectile::Type ) && ( dropOnly || drag.GetDistanceToGoal() > DRAG_FAIL_LEN ) ) {
			...
			if (holdingAF) {
				...
				bool oldTruncateImpulse = afPhysics->cstTruncateImpulse;
				afPhysics->cstTruncateImpulse = false;
				ent->ApplyImpulse(thePlayer, 0, ent->GetPhysics()->GetOrigin(), thePlayer->firstPersonViewAxis[0] * THROW_SCALE * ent->GetPhysics()->GetMass());
				afPhysics->cstTruncateImpulse = oldTruncateImpulse;
			} else {
				ent->ApplyImpulse(thePlayer, 0, ent->GetPhysics()->GetOrigin(), thePlayer->firstPersonViewAxis[0] * THROW_SCALE * ent->GetPhysics()->GetMass());
			}
			//#modified-fva; END
			...
			if ( ent->IsType( idProjectile::Type ) ) {
				...
				if ( projectile != NULL ) {
					...
					//#modified-fva; BEGIN
					projectile->CstReleaseProjectile(thePlayer);
					//#modified-fva; END
				}
...
int idGrabber::Update( idPlayer *player, bool hide ) {
	...
	//#modified-fva; END
		...
		*/
		// Dropping an item with "reload" is now signaled via idGrabber::CstReload and idWeapon::Reload.
		if (cstDrop) {
			abort = true;
		}
		//#modified-fva; END
	...
	if ( dragEnt.GetEntity() && allow ) {
		...
		if (!cst_grabberUnlimitedTimeSP.GetBool() && g_grabberEnableShake.GetBool() && !common->IsMultiplayer()) {
		//#modified-fva; END
			ApplyShake();
		}
