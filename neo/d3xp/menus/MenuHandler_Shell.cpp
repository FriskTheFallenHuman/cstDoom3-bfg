...
bool idMenuHandler_Shell::HandleGuiEvent( const sysEvent_t * sev ) {
	...
	if ( showingIntro ) {
		//#modified-fva; BEGIN
		if (sev->evType == SE_KEY && sev->evValue2 == 1 && (sev->evValue == K_ESCAPE || sev->evValue == K_JOY9) && introGui) {
			introGui->SetGlobal("cst_skip", true);
		}
		//#modified-fva; END
		return true;
	}
	...
	if (inGame && sev->evType == SE_KEY && sev->evValue2 == 1 && sev->evValue >= K_F1 && sev->evValue <= K_F12) {
		const char * binding = idKeyInput::GetBinding(sev->evValue, 0, 0);
	...
	if ( waitForBinding ) {
		...
		if ( sev->evType == SE_KEY && sev->evValue2 == 1 ) {
			...
			//#modified-fva; BEGIN
			idMenuScreen_Shell_CstBindingsBase *bindScreen = dynamic_cast<idMenuScreen_Shell_CstBindingsBase *>(menuScreens[cstWaitShellArea]);
			//#modified-fva; END
			...
			if ( sev->evValue == K_ESCAPE ) {
				...
				//#modified-fva; BEGIN
				if (idStr::Cmp(waitBind, "cst_keyPdaScoreClose") == 0) {
					const char *keyName = cst_keyPdaScoreClose.GetString();
					keyNum_t keyNum = idKeyInput::StringToKeyNum(keyName);
					if (sev->evValue == keyNum) {
						// same key; "unbind" it
						cst_keyPdaScoreClose.SetString("");
					} else {
						// new key; "bind" it
						const char *newKeyName = idKeyInput::KeyNumToString((keyNum_t)sev->evValue);
						cst_keyPdaScoreClose.SetString(newKeyName);
					}
					if (bindScreen != NULL) {
				...
				int cstDomain = 0;
				if (bindScreen) {
					cstLayer = bindScreen->GetLayer();
					cstDomain = bindScreen->GetDomain();
				}
				if (idStr::Icmp(idKeyInput::GetBinding(sev->evValue, cstLayer, cstDomain), "") == 0) {
				//#modified-fva; END

					//#modified-fva; BEGIN
					//idKeyInput::SetBinding( sev->evValue, waitBind );
					//idMenuScreen_Shell_Bindings * bindScreen = dynamic_cast< idMenuScreen_Shell_Bindings * >( menuScreens[ SHELL_AREA_KEYBOARD ] );
					idKeyInput::SetBinding(sev->evValue, waitBind, cstLayer, cstDomain);
					//#modified-fva; END
					if ( bindScreen != NULL ) {
						bindScreen->SetBindingChanged( true );
						bindScreen->UpdateBindingDisplay();
					...
					const char * curBind = idKeyInput::GetBinding(sev->evValue, cstLayer, cstDomain);
					//#modified-fva; END
					...
					if ( idStr::Icmp( waitBind, curBind ) == 0 ) {
						...
						//idMenuScreen_Shell_Bindings * bindScreen = dynamic_cast< idMenuScreen_Shell_Bindings * >( menuScreens[ SHELL_AREA_KEYBOARD ] );
						idKeyInput::SetBinding(sev->evValue, "", cstLayer, cstDomain);
						//#modified-fva; END
						if ( bindScreen != NULL ) {
							bindScreen->SetBindingChanged( true );
							bindScreen->UpdateBindingDisplay();
						...
						if ( bindScreen != NULL ) {
							...
							public:
								...
								idSWFScriptFunction_RebindKey(idMenuScreen_Shell_CstBindingsBase * _menu, gameDialogMessages_t _msg, bool _accept, idMenuHandler_Shell * _mgr, int _key, const char * _bind) {
								//#modified-fva; END
									menu = _menu;
									msg = _msg;
									accept = _accept;
								...
								idSWFScriptVar Call( idSWFScriptObject * thisObject, const idSWFParmList & parms ) {
									...
									if ( accept ) {
										...
										idKeyInput::SetBinding(key, bind, menu->GetLayer(), menu->GetDomain());
										//#modified-fva; END
							...
							private:
								...
								idMenuScreen_Shell_CstBindingsBase * menu;
								//#modified-fva; END
...
#define BIND_SHELL_SCREEN( screenId, className, menuHandler )				\
	...
	if ( inGame ) {
		...
		//#modified-fva; BEGIN
		BIND_SHELL_SCREEN(SHELL_AREA_SETTINGS, idMenuScreen_Shell_CstSettings, this);
		BIND_SHELL_SCREEN(SHELL_AREA_CONTROLS, idMenuScreen_Shell_CstControls, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_BINDINGS_DOOM3, idMenuScreen_Shell_CstBindingsDoom3, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_BINDINGS_DOOM_CLASSIC_NORMAL, idMenuScreen_Shell_CstBindingsDoomClassicNormal, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_BINDINGS_DOOM_CLASSIC_AUTOMAP, idMenuScreen_Shell_CstBindingsDoomClassicAutomap, this);
		BIND_SHELL_SCREEN(SHELL_AREA_GAMEPAD, idMenuScreen_Shell_CstGamepad, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_GAME_OPTIONS_DOOM3, idMenuScreen_Shell_CstGameOptionsDoom3, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_GO_D3_GENERAL_OPTIONS_1, idMenuScreen_Shell_Cst_GoD3_GeneralOptions1, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_GO_D3_GENERAL_OPTIONS_2, idMenuScreen_Shell_Cst_GoD3_GeneralOptions2, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_GO_D3_GENERAL_OPTIONS_3, idMenuScreen_Shell_Cst_GoD3_GeneralOptions3, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_GO_D3_RUN_CROUCH_ZOOM, idMenuScreen_Shell_Cst_GoD3_RunCrouchZoom, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_GO_D3_MAP_OPTIONS, idMenuScreen_Shell_Cst_GoD3_MapOptions, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_GO_D3_WEAPON_OPTIONS, idMenuScreen_Shell_Cst_GoD3_WeaponOptions, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_GO_D3_WEAPON_SMOKE_SELF, idMenuScreen_Shell_Cst_GoD3_WeaponSmokeSelf, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_GO_D3_WEAPON_SMOKE_OTHERS_MP, idMenuScreen_Shell_Cst_GoD3_WeaponSmokeOthersMP, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_GO_D3_PROJECTILE_SMOKE_SELF, idMenuScreen_Shell_Cst_GoD3_ProjectileSmokeSelf, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_GO_D3_PROJECTILE_SMOKE_OTHERS_MP, idMenuScreen_Shell_Cst_GoD3_ProjectileSmokeOthersMP, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_GO_D3_DAMAGE_FEEDBACK, idMenuScreen_Shell_Cst_GoD3_DamageFeedback, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_GO_D3_SERVER_OPTIONS, idMenuScreen_Shell_Cst_GoD3_ServerOptions, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_GAME_OPTIONS_DOOM_CLASSIC, idMenuScreen_Shell_CstGameOptionsDoomClassic, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_GO_DC_GENERAL_OPTIONS_1, idMenuScreen_Shell_Cst_GoDC_GeneralOptions1, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_GO_DC_AUTOMAP_OPTIONS, idMenuScreen_Shell_Cst_GoDC_AutomapOptions, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_GO_DC_RUN_OPTIONS, idMenuScreen_Shell_Cst_GoDC_RunOptions, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_GO_DC_SCREEN_OPTIONS, idMenuScreen_Shell_Cst_GoDC_ScreenOptions, this);
		BIND_SHELL_SCREEN(SHELL_AREA_RESOLUTION, idMenuScreen_Shell_CstResolution, this);
		BIND_SHELL_SCREEN(SHELL_AREA_SYSTEM_OPTIONS, idMenuScreen_Shell_CstSystemOptions1, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_SYSTEM_OPTIONS_2, idMenuScreen_Shell_CstSystemOptions2, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_PRESETS, idMenuScreen_Shell_CstPresets, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_ACHIEVEMENTS, idMenuScreen_Shell_CstAchievements, this);
		BIND_SHELL_SCREEN(CST_SHELL_AREA_HELP, idMenuScreen_Shell_CstHelp, this);
		//#modified-fva; END
...
enum shellCommandsPC_t {
	...
	SHELL_CMD_DEMO1,
	//#modified-fva; BEGIN
	// dev menu - moved to MenuScreen_Shell_Singleplayer.cpp
	//SHELL_CMD_DEV,
	//#modified-fva; END
	SHELL_CMD_CAMPAIGN,
	SHELL_CMD_MULTIPLAYER,
	//#modified-fva; BEGIN
	CST_SHELL_CMD_ACHIEVEMENTS,
	//#modified-fva; END
	SHELL_CMD_SETTINGS,
	SHELL_CMD_CREDITS,
	SHELL_CMD_QUIT
	...
	if ( GetPlatform() == 2 && menuBar != NULL ) {
		if ( g_demoMode.GetBool() ) {
			...
			*/
			...
			//#modified-fva; BEGIN
			if (common->CstGetLatchedLanguage().Icmp(ID_LANG_ITALIAN) == 0) {
				navOptions.Append(CST_STR_00300397); // achievements (truncated label)
			} else {
				navOptions.Append(CST_STR_00300363); // achievements
			}
			//#modified-fva; END
			navOptions.Append( "#str_swf_settings" );	// settings
			...
			*/
			...
			if (buttonWidget != NULL) {
				buttonWidget->ClearEventActions();
				buttonWidget->AddEventAction(WIDGET_EVENT_PRESS).Set(WIDGET_ACTION_COMMAND, CST_SHELL_CMD_ACHIEVEMENTS, index);
				buttonWidget->SetDescription(CST_STR_00300367);
			}
			//#modified-fva; END
			index++;
			buttonWidget = dynamic_cast< idMenuWidget_MenuButton * >( &menuBar->GetChildByIndex( index ) );
		...
		case WIDGET_ACTION_COMMAND: {
			...
			*/
			if (cmd != SHELL_CMD_QUIT && (
				...
				nextScreen == CST_SHELL_AREA_GO_D3_GENERAL_OPTIONS_1 ||
				nextScreen == CST_SHELL_AREA_GO_D3_GENERAL_OPTIONS_2 ||
				nextScreen == CST_SHELL_AREA_GO_D3_GENERAL_OPTIONS_3 ||
				nextScreen == CST_SHELL_AREA_GO_D3_RUN_CROUCH_ZOOM ||
				nextScreen == CST_SHELL_AREA_GO_D3_MAP_OPTIONS ||
				nextScreen == CST_SHELL_AREA_GO_D3_WEAPON_OPTIONS ||
				nextScreen == CST_SHELL_AREA_GO_D3_WEAPON_SMOKE_SELF ||
				nextScreen == CST_SHELL_AREA_GO_D3_WEAPON_SMOKE_OTHERS_MP ||
				nextScreen == CST_SHELL_AREA_GO_D3_PROJECTILE_SMOKE_SELF ||
				nextScreen == CST_SHELL_AREA_GO_D3_PROJECTILE_SMOKE_OTHERS_MP ||
				nextScreen == CST_SHELL_AREA_GO_D3_DAMAGE_FEEDBACK ||
				nextScreen == CST_SHELL_AREA_GO_D3_SERVER_OPTIONS ||
				nextScreen == CST_SHELL_AREA_GO_DC_GENERAL_OPTIONS_1 ||
				nextScreen == CST_SHELL_AREA_GO_DC_AUTOMAP_OPTIONS ||
				nextScreen == CST_SHELL_AREA_GO_DC_RUN_OPTIONS ||
				nextScreen == CST_SHELL_AREA_GO_DC_SCREEN_OPTIONS ||
				nextScreen == SHELL_AREA_SYSTEM_OPTIONS ||
				nextScreen == CST_SHELL_AREA_SYSTEM_OPTIONS_2 ||
				nextScreen == CST_SHELL_AREA_PRESETS)) {
				cvarSystem->SetModifiedFlags(CVAR_ARCHIVE);
			}
			//#modified-fva; END
			...
			switch ( cmd ) {
				...
				*/
				...
				//#modified-fva; BEGIN
				case CST_SHELL_CMD_ACHIEVEMENTS: {
					nextScreen = CST_SHELL_AREA_ACHIEVEMENTS;
					transition = MENU_TRANSITION_SIMPLE;
					break;
				}
				//#modified-fva; END
	...
	*/
	...
	if (smallFrameShowing) {
		if (nextScreen != SHELL_AREA_DEV &&
			...
			nextScreen != CST_SHELL_AREA_GO_D3_DAMAGE_FEEDBACK &&
			nextScreen != CST_SHELL_AREA_GO_D3_SERVER_OPTIONS &&
			nextScreen != CST_SHELL_AREA_GAME_OPTIONS_DOOM_CLASSIC &&
			nextScreen != CST_SHELL_AREA_GO_DC_GENERAL_OPTIONS_1 &&
			nextScreen != CST_SHELL_AREA_GO_DC_AUTOMAP_OPTIONS &&
			nextScreen != CST_SHELL_AREA_GO_DC_RUN_OPTIONS &&
			nextScreen != CST_SHELL_AREA_GO_DC_SCREEN_OPTIONS &&
			nextScreen != SHELL_AREA_RESOLUTION &&
			nextScreen != CST_SHELL_AREA_SYSTEM_OPTIONS_2 &&
			nextScreen != CST_SHELL_AREA_PRESETS) {
			ShowSmallFrame(false);
		}
	...
	if (largeFrameShowing) {
		if (nextScreen != SHELL_AREA_LOAD &&
			nextScreen != SHELL_AREA_SAVE &&
			nextScreen != SHELL_AREA_GAME_LOBBY &&
			nextScreen != SHELL_AREA_PARTY_LOBBY &&
			...
			nextScreen != SHELL_AREA_BROWSER &&
			nextScreen != SHELL_AREA_CREDITS &&
			nextScreen != CST_SHELL_AREA_BINDINGS_DOOM3 &&
			nextScreen != CST_SHELL_AREA_BINDINGS_DOOM_CLASSIC_NORMAL &&
			nextScreen != CST_SHELL_AREA_BINDINGS_DOOM_CLASSIC_AUTOMAP &&
			nextScreen != CST_SHELL_AREA_GAME_OPTIONS_DOOM3 &&
			nextScreen != CST_SHELL_AREA_GO_D3_WEAPON_OPTIONS &&
			nextScreen != CST_SHELL_AREA_GO_D3_WEAPON_SMOKE_SELF &&
			nextScreen != SHELL_AREA_SYSTEM_OPTIONS &&
			nextScreen != CST_SHELL_AREA_ACHIEVEMENTS &&
			nextScreen != CST_SHELL_AREA_HELP) {
			ShowMPFrame(false);
		}
	} else {
		if (nextScreen == SHELL_AREA_LOAD ||
...
*/
 void idMenuHandler_Shell::ShowDoomIntro() {
	...
	if ( introGui != NULL ) {
		//#modified-fva; BEGIN
		introGui->SetGlobal("cst_skip", false);
		introGui->SetGlobal("cst_skipping", false);
		//#modified-fva; END
			...
			if ( infoSprite != NULL && txtVal != NULL ) {
				...
				public:
					...
					idSWFScriptVar Call( idSWFScriptObject * thisObject, const idSWFParmList & parms ) {
						...
						//#modified-fva; BEGIN
						if (gui->GetGlobal("cst_skipping").ToBool()) {
							txtVal->ClearPlaySound();
							txtVal->soundClip = "";
						} else if (gui->GetGlobal("cst_skip").ToBool()) {
							gui->SetGlobal("cst_skipping", true);
							txtVal->ClearPlaySound();
							txtVal->soundClip = "";
							shell->StartGame(0);
						}
							...
							if ( !txtVal->generatingText ) {
								...
								if ( newYPos <= 350.0f - ( numLines * 36.0f ) ) {
									if ( thisObject->GetSprite()->IsVisible() ) {
										...
										*/
										if (!gui->GetGlobal("cst_skipping").ToBool() && nextIndex >= NUM_DOOM_INTRO_LINES) {
											gui->SetGlobal("cst_skipping", true);
											txtVal->ClearPlaySound();
											txtVal->soundClip = "";
											shell->StartGame(0);
											...
											if ( txtData != NULL && !txtData->generatingText ) {
												if ( startFade == 0 ) {
													...
													if ( Sys_Milliseconds() - startFade >= 3000 ) {
														...
														*/
														} else if (!gui->GetGlobal("cst_skipping").ToBool()) {
															gui->SetGlobal("cst_skipping", true);
															txtVal->ClearPlaySound();
															txtVal->soundClip = "";
															shell->StartGame(1);
	...
	if ( introGui != NULL ) {
		...
		if ( infoSprite != NULL ) {
			...
			public:
				...
				idIntroTextUpdate(idSWFTextInstance * _txtVal, idMenuHandler_Shell * _shell, idSWF * _gui) {
				//#modified-fva; END
					txtVal = _txtVal;
					generating = false;
					shell = _shell;
			...
			infoSprite->GetScriptObject()->Set("onEnterFrame", new idIntroTextUpdate(txtVal, this, introGui));
			//#modified-fva; END
		}
	}
}
