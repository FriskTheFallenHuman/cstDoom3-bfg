...
void idMenuScreen_HUD::ShowScreen( const mainMenuTransition_t transitionType ) {
	...
	//#modified-fva; BEGIN
	cstRunIndicator = root.GetNestedSprite("_absBottomLeft", "cstRunIndicator");
	cstCrouchIndicator = root.GetNestedSprite("_absBottomLeft", "cstCrouchIndicator");
	cstZoomIndicator = root.GetNestedSprite("_absBottomLeft", "cstZoomIndicator");
	cstAccessCode = root.GetNestedSprite("_absTopRight", "cstAccessCode");
	//#modified-fva; END
	...
	if (!common->IsMultiplayer()) {
		...
		if (player && player->hud == this) {
			if (cstUpdateOnRestore) {
				cstUpdateOnRestore = false;
				if (player->IsTipVisible()) {
					player->CstShowLastTip();
				}
				if (player->CstIsObjectiveUp()) {
					ShowObjective(false);
				}
				if (player->CstIsObjectiveCompleteUp()) {
					ShowObjective(true);
				}
				if (player->CstIsNewVideoUp()) {
					ToggleNewVideo(true);
				}
				if (player->CstIsNewPDAUp()) {
					ToggleNewPDA(true);
				}
			}
...
void idMenuScreen_HUD::UpdateHealthArmor( idPlayer * player ) {
	...
	if ( healthPulse != NULL ) {
		if ( player->healthPulse ) {
			...
			if (player->cstPlayHealthPulseSound) {
				player->cstPlayHealthPulseSound = false;
				player->StartSound("snd_healthpulse", SND_CHANNEL_ITEM, 0, false, NULL);
			}
			//#modified-fva; END
...
void idMenuScreen_HUD::UpdateWeaponInfo( idPlayer * player ) {
	...
	if ( ammoAmount == -1 || player->GetCurrentWeaponSlot() == player->weapon_bloodstone || player->GetCurrentWeaponSlot() == player->weapon_soulcube ) {
		...
		//#modified-fva; BEGIN
		{
			bool smartAmmo = common->IsMultiplayer() ? cst_hudSmartAmmoMP.GetBool() : cst_hudSmartAmmoSP.GetBool();
			if (smartAmmo && weapon.GetEntity()->ClipSize() > 0 && weapon.GetEntity()->GetRenderEntity()->gui[0] != NULL) {
				ammoInfo->SetVisible(false);
				return; // watch out
			}
		}
				...
				if ( talkCursor ) {
					...
					if ( txtAction ) {
						...
						if ( !in_useJoystick.GetBool() ) {
							...
							keyBindings_t bind = idKeyInput::KeyBindingsFromBinding("_use", 0, 0, true);
							//#modified-fva; END
...
void idMenuScreen_HUD::UpdateWeaponStates( idPlayer * player, bool weaponChanged ) {
	...
	if ( common->IsMultiplayer() ) {
		...
		if (!mpWeapons || player->GetIdealWeapon() == 0 || player->GetIdealWeapon() == 16) {
		//#modified-fva; END
			return;
		}
		...
		if ( weaponChanged ) {
			...
			for ( int i = 1; i < MAX_WEAPONS; ++i ) {
				//#modified-fva; BEGIN
				if (i >= 16 && i <= 18) {
					continue; // skip the handheld flashlight, the armor-mounted flashlight, and the PDA
				}
				//#modified-fva; END
...
//#modified-fva; BEGIN
void idMenuScreen_HUD::CstUpdateEnviro(bool show, int bar, int val) {
	...
	if (show) {
		...
		if (txtVal != NULL) {
			//txtVal->SetText("#str_00100922");
			txtVal->SetText(CST_STR_HUD_ENVIRO);
			txtVal->SetStrokeInfo(true, 0.9f, 2.0f);
		}
...
//#modified-fva; BEGIN
void idMenuScreen_HUD::CstUpdateZoomIndicator(bool show) {
	if (cstZoomIndicator) {
		if (show && !cstZoomIndicator->IsVisible()) {
			cstZoomIndicator->SetVisible(true);
		} else if (!show && cstZoomIndicator->IsVisible()) {
			cstZoomIndicator->SetVisible(false);
		}
	}
}

// ===============
void idMenuScreen_HUD::CstUpdateCrouchIndicator(bool show) {
	if (cstCrouchIndicator) {
		if (show && !cstCrouchIndicator->IsVisible()) {
			cstCrouchIndicator->SetVisible(true);
		} else if (!show && cstCrouchIndicator->IsVisible()) {
			cstCrouchIndicator->SetVisible(false);
		}
	}
}

// ===============
void idMenuScreen_HUD::CstUpdateRunIndicator(bool show) {
	if (cstRunIndicator) {
		if (show && !cstRunIndicator->IsVisible()) {
			cstRunIndicator->SetVisible(true);
		} else if (!show && cstRunIndicator->IsVisible()) {
			cstRunIndicator->SetVisible(false);
		}
	}
}

// ===============
void idMenuScreen_HUD::CstShowAccessCode(const idStr &code) {
	if (cstAccessCode) {
		idSWFTextInstance *txtVal = cstAccessCode->GetScriptObject()->GetText("txtVal");
		if (txtVal) {
			txtVal->SetText(code);
			txtVal->SetStrokeInfo(true, 0.75f, 1.5f);
		}
		if (!cstAccessCode->IsVisible()) {
			cstAccessCode->SetVisible(true);
		}
	}
}

// ===============
void idMenuScreen_HUD::CstHideAccessCode() {
	if (cstAccessCode && cstAccessCode->IsVisible()) {
		cstAccessCode->SetVisible(false);
	}
}
//#modified-fva; END
