	...
	if ( menuData != NULL && menuData->GetPlatform() != 2 ) {
		...
		if ( g_demoMode.GetBool() ) {
			...
			*/
			...
			//#modified-fva; BEGIN
			option.Clear();
			option.Append(CST_STR_00300363);	// achievements
			menuOptions.Append(option);
			options->GetChildByIndex(index).ClearEventActions();
			options->GetChildByIndex(index).AddEventAction(WIDGET_EVENT_PRESS).Set(WIDGET_ACTION_COMMAND, CST_ROOT_CMD_ACHIEVEMENTS);
			buttonWidget = dynamic_cast<idMenuWidget_Button *>(&options->GetChildByIndex(index));
			if (buttonWidget != NULL) {
		...
		case WIDGET_ACTION_COMMAND: {
			switch ( parms[0].ToInteger() ) {
				...
				*/
				...
				//#modified-fva; BEGIN
				case CST_ROOT_CMD_ACHIEVEMENTS: {
					menuData->SetNextScreen(CST_SHELL_AREA_ACHIEVEMENTS, MENU_TRANSITION_SIMPLE);
					break;
				}
				//#modified-fva; END
