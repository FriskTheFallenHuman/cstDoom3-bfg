		...
		if ( nextScreen == SCOREBOARD_AREA_INVALID ) {
			...
			if ( nextScreen > SCOREBOARD_AREA_INVALID && nextScreen < SCOREBOARD_NUM_AREAS && menuScreens[ nextScreen ] != NULL ) {
				...
				if (verify(shortcutKeys != NULL)) {
					idStr keyName = cst_keyPdaScoreClose.GetString();
					if (!keyName.IsEmpty() && idKeyInput::StringToKeyNum(keyName.c_str()) != K_NONE) {
						keyName.ToUpper();
						if (keyName != "TAB") {
							shortcutKeys->Set(keyName.c_str(), "TAB");
						}
					}
				}
				//#modified-fva; END
...
bool idMenuHandler_Scoreboard::HandleGuiEvent(const sysEvent_t * sev) {
	if (sev->evType == SE_KEY && sev->evValue2 == 1 && sev->evValue >= K_F1 && sev->evValue <= K_F12) {
		if (idStr::Icmp(idKeyInput::GetBinding(sev->evValue, 0, 0), "screenshot") == 0) {
