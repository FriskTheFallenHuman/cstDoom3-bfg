...
void idMenuWidget_Shell_SaveInfo::Update() {
	...
	if ( loadIndex >= 0 && sortedSaves.Num() != 0 && loadIndex < sortedSaves.Num() ) {
		...
		//#modified-fva; BEGIN
		idStr cstAwStr;
		if (details.GetExpansion() == GAME_BASE) {
			int dbsEnabled = details.CstGetAwDbsD3();
			int grabberEnabled = details.CstGetAwGrabberD3();
			if (dbsEnabled > -1 && grabberEnabled > -1) {
				if (dbsEnabled > 0 && grabberEnabled > 0) {
					cstAwStr = idLocalization::GetString(CST_STR_AW_DBS_GRABBER);
				} else if (dbsEnabled > 0) {
					cstAwStr = idLocalization::GetString(CST_STR_AW_DBS);
				} else if (grabberEnabled > 0) {
					cstAwStr = idLocalization::GetString(CST_STR_AW_GRABBER);
				}
			}
		} else if (details.GetExpansion() == GAME_D3XP || details.GetExpansion() == GAME_D3LE) {
			int chainsawEnabled = details.GetExpansion() == GAME_D3XP ? details.CstGetAwChainsawRoE() : details.CstGetAwChainsawLM();
			if (chainsawEnabled > 0) {
				cstAwStr = idLocalization::GetString(CST_STR_AW_CHAINSAW);
			}
		}
		if (!cstAwStr.IsEmpty()) {
		...
		} else if (details.GetSaveVersion() != CST_SAVEGAME_VERSION) {
		//#modified-fva; END
			info.Append( "\n" );
