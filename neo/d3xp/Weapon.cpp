...
const idEventDef EV_Weapon_CstGetAutoReloadDbs("cstGetAutoReloadDbs", NULL, 'd');
const idEventDef EV_Weapon_CstGetInstaReload("cstGetInstaReload", NULL, 'd');
//#modified-fva; END
...
CLASS_DECLARATION( idAnimatedEntity, idWeapon )
	...
	EVENT(EV_Weapon_CstGetAutoReloadDbs, idWeapon::Event_CstGetAutoReloadDbs)
	EVENT(EV_Weapon_CstGetInstaReload, idWeapon::Event_CstGetInstaReload)
	//#modified-fva; END
END_CLASS
...
void idWeapon::PresentWeapon( bool showViewModel ) {
	...
	if (common->IsClient() && IsLinked() && cstCanSyncLight && cstSnapshotLight > 0) {
		bool cstLightOn = (bool)(cstSnapshotLight - 1);
		if (owner != NULL && !owner->IsLocallyControlled() && cstLightOn != lightOn) {
			if (isPlayerFlashlight) {
...
//		adjustAng += owner->GetViewBobAngles();
	...
	{
		int currentWeapon = owner->GetCurrentWeaponSlot();
		if (currentWeapon >= 0) {
			bool isAllowed = true;
			if (!common->IsMultiplayer() || gameLocal.cstSiAllowSmokeControlMP) {
				idCVar *noSmokeMuzzleCVar;
				if (common->IsMultiplayer() && owner->entityNumber != gameLocal.GetLocalClientNum()) {
...
void idWeapon::Event_SetSkin( const char *skinname ) {
	...
	if (common->IsServer() && !isPlayerFlashlight && owner && owner->GetCurrentWeaponSlot() != owner->weapon_flashlight) {
	//#modified-fva; END
		idBitMsg			msg;
...
void idWeapon::Event_LaunchProjectiles( int num_projectiles, float spread, float fuseOffset, float launchPower, float dmgPower ) {
	...
	const bool cstForceReplication = projectileDict.GetBool("cst_net_replicate");
	const bool actuallySpawnProjectile = common->IsServer() || (attackerIsLocal && !cstForceReplication) || isHitscan;
	//#modified-fva; END
	...
	if ( actuallySpawnProjectile ) {
		...
		//#modified-fva; BEGIN
		if (!common->IsMultiplayer() && owner->GetCurrentWeaponSlot() == owner->cst_weapon_shotgun && cst_shotgunHalfSpreadSP.GetBool()) {
			spread *= 0.5f;
		}
		//#modified-fva; END
...
// ==========
void idWeapon::Event_CstGetAutoReloadDbs() {
	// code below is adapted from idWeapon::Event_AutoReload

	assert(owner);
	if (!owner || (common->IsClient() && !owner->IsLocallyControlled())) {
		idThread::ReturnInt(false);
		return;
	}
	idLobbyBase &lobby = session->GetActingGameStateLobbyBase();
	lobbyUserID_t &lobbyUserID = gameLocal.lobbyUserIDs[owner->entityNumber];
	bool autoReload = lobby.GetLobbyUserWeaponAutoReload(lobbyUserID);
	if (autoReload) {
		idThread::ReturnInt(true);
		return;
	}
	bool autoReloadAlwaysDbs = lobby.CstGetLobbyUserAutoReloadAlwaysDbs(lobbyUserID);
	idThread::ReturnInt(autoReloadAlwaysDbs);
}

// ==========
void idWeapon::Event_CstGetInstaReload() {
	if (common->IsMultiplayer()) {
		idThread::ReturnInt(false);
		return;
	}
	idThread::ReturnInt(cst_instaReloadSP.GetBool());
}
//#modified-fva; END
