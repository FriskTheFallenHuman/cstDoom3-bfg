...
bool idSmokeParticles::EmitSmoke( const idDeclParticle *smoke, const int systemStartTime, const float diversity, const idVec3 &origin, const idMat3 &axis, int timeGroup /*_D3XP*/ ) {
	...
	//#modified-fva; BEGIN
	activeSmokeStage_t cstLocalActiveStages[MAX_PARTICLE_STAGES];
	idStaticList<int, MAX_PARTICLE_STAGES> cstActiveIndices;
	//#modified-fva; END
		...
		for ( i = 0 ; i < activeStages.Num() ; i++ ) {
			...
			*/
			if (activeStages[i].stage == stage) {
				cstActiveIndices.Append(i);
				active = &cstLocalActiveStages[cstActiveIndices.Num() - 1];
	...
	//#modified-fva; BEGIN
	if (cstActiveIndices.Num() > 1) {
		cstActiveIndices.Sort();
	}
	for (int i = 0; i < cstActiveIndices.Num(); ++i) {
		int index = cstActiveIndices[i];
		activeStages[index] = cstLocalActiveStages[i];
	}
	//#modified-fva; END
