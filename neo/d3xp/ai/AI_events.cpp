...
const idEventDef AI_CstFindHittableEnemy("cstFindHittableEnemy", "d", 'e');
const idEventDef AI_CstFindHittableEnemyAI("cstFindHittableEnemyAI", "d", 'e');
//#modified-fva; END
...
CLASS_DECLARATION( idActor, idAI )
	...
	EVENT(AI_CstFindHittableEnemy, idAI::Event_CstFindHittableEnemy)
	EVENT(AI_CstFindHittableEnemyAI, idAI::Event_CstFindHittableEnemyAI)
	//#modified-fva; END
END_CLASS
...
// ===============
bool idAI::CstCanHitActor(const idActor &actor, bool useFov) {
...
// ===============
void idAI::Event_CstFindHittableEnemy(int useFov) {
	...
	if (gameLocal.InPlayerPVS(this)) {
		for (int i = 0; i < gameLocal.numClients; i++) {
			...
			if (CstCanHitActor(*actor, useFov != 0)) {
				idThread::ReturnEntity(actor);
				return;
			}
		}
...
// ===============
void idAI::Event_CstFindHittableEnemyAI(int useFov) {
	...
	for (idEntity *ent = gameLocal.activeEntities.Next(); ent != NULL; ent = ent->activeNode.Next()) {
		...
		if ((dist < bestDist) && CstCanHitActor(*actor, useFov != 0)) {
			bestDist = dist;
			bestEnemy = actor;
		}
	}
