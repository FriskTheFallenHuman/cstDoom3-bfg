...
void idProjectile::CatchProjectile( idEntity* o, const char* reflectName ) {
	...
	//#modified-fva; BEGIN
	cstSmokeFlyControl.OnGrabberCatch(this);
	thrust = 0.0f;
	//#modified-fva; END
...
void idProjectile::CstSmokeFlyControl::Init() {
	...
	const char *playerDefName;
	if (common->IsMultiplayer()) {
		playerDefName = gameLocal.GetMPPlayerDefName();
	} else {
		playerDefName = gameLocal.world->spawnArgs.GetString("def_player", "player_doommarine");
	}
	const idDeclEntityDef *playerDef = gameLocal.FindEntityDef(playerDefName, false);
	if (!playerDef) {
		return;
	}
...
idCVar & idProjectile::CstSmokeFlyControl::GetNoSmokeFlyCVar(idPlayer &player) {
	...
	if (common->IsMultiplayer() && player.entityNumber != gameLocal.GetLocalClientNum()) {
...
// ==============
void idProjectile::CstSmokeFlyControl::OnGrabberCatch(idProjectile *projectile) {
	if (common->IsMultiplayer()) {
		return;
	}

	if (cst_noSmokeFlyGrabbed.GetBool()) {
...
void idProjectile::CstSmokeFlyControl::OnGrabberRelease(idPlayer *grabberOwner, idProjectile *projectile) {
	if (common->IsMultiplayer() || !grabberOwner) {
		return;
	}
	...
	if ( state == LAUNCHED ) {
		...
		for ( int i = 0; i < beamTargets.Num(); i++ ) {
			if ( beamTargets[i].target.GetEntity() == NULL ) {
				//#modified-fva; BEGIN
				CstFreeSingleBeam(beamTargets[i]);
				if (!common->IsClient() && beamTargets[i].cstIsPlayer) {
...
//#modified-fva; BEGIN
void idHomingProjectile::CstUnGuide() {
	unGuided = true;
}
//#modified-fva; END
