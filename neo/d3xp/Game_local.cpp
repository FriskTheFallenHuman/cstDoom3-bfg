...
void idGameLocal::Clear() {
	...
	cstSiAllowHandFlashlightMP = false;
	cstSiOldPlayerSpeedMP = false;
	cstSiOldAmmoMP = false;
...
void idGameLocal::SaveGame( idFile *f, idFile *strings ) {
	...
	idSaveGame savegame(f, strings, CST_SAVEGAME_VERSION);
	//#modified-fva; END
...
void idGameLocal::GetSaveGameDetails( idSaveGameDetails & gameDetails ) {
	...
	gameDetails.descriptors.SetInt(SAVEGAME_DETAIL_FIELD_SAVE_VERSION, CST_SAVEGAME_VERSION);
	//#modified-fva; END
	...
	//#modified-fva; BEGIN
	gameDetails.descriptors.SetInt(SAVEGAME_DETAIL_FIELD_CST_AW_DBS_D3, player ? player->cstAwDoubleBarrelShotgunD3 : -1);
	gameDetails.descriptors.SetInt(SAVEGAME_DETAIL_FIELD_CST_AW_GRABBER_D3, player ? player->cstAwGrabberD3 : -1);
	gameDetails.descriptors.SetInt(SAVEGAME_DETAIL_FIELD_CST_AW_CHAINSAW_ROE, player ? player->cstAwChainsawRoE : -1);
	gameDetails.descriptors.SetInt(SAVEGAME_DETAIL_FIELD_CST_AW_CHAINSAW_LM, player ? player->cstAwChainsawLM : -1);
	//#modified-fva; END
...
void idGameLocal::SetServerInfo( const idDict &_serverInfo ) {
	...
	cstSiAllowHandFlashlightMP = serverInfo.GetBool("cst_allowHandFlashlightMP");
	cstSiOldPlayerSpeedMP = serverInfo.GetBool("cst_oldPlayerSpeedMP");
	cstSiOldAmmoMP = serverInfo.GetBool("cst_oldAmmoMP");
...
void idGameLocal::LoadMap( const char * mapName, int randseed ) {
	...
	//#modified-fva; BEGIN
	FindEntityDef("cst_old_healthstation", false);
	FindEntityDef("cst_access_codes", false);
	FindEntityDef("cst_player_headlamp", false);
	FindEntityDef("cst_underground_grabber_1", false);
	FindEntityDef("cst_underground_shotgun_double_1", false);
	FindEntityDef("cst_hell1_grabber_1", false);
	FindEntityDef("cst_hell1_shotgun_double_1", false);
	FindEntityDef("cst_delta5_grabber_1", false);
	FindEntityDef("cst_delta5_shotgun_double_1", false);
	FindEntityDef("cst_erebus1_chainsaw_1", false);
	FindEntityDef("cst_le_enpro1_chainsaw_1", false);
	//#modified-fva; END
...
bool idGameLocal::InitFromSaveGame( const char *mapName, idRenderWorld *renderWorld, idSoundWorld *soundWorld, idFile * saveGameFile, idFile * stringTableFile, int saveGameVersion ) {
	...
	//#modified-fva; BEGIN
	idProjectile::CstInitSmokeFlyControl();
	CstAdjustCinematicUpdateListTime(fast.time);
	//#modified-fva; END
...
int idGameLocal::GetLocalClientNum() const {
	//#modified-fva; BEGIN
	if (common->IsServer() && common->CstGetServerDedicated()) {
		return -1;
	}
	//#modified-fva; END
...
void idGameLocal::RunFrame( idUserCmdMgr & cmdMgr, gameReturn_t & ret ) {
	...
	//#modified-fva; BEGIN
	const int cstInitialFrame = framenum + 1;
	//#modified-fva; END
	...
	// moved to idCommonLocal::Frame as CstSyncPlayersWithLobbyUsers
	//#modified-fva; END
	...
	//#modified-fva; from doom3 gpl; END
		...
		//#modified-fva; BEGIN
		if (framenum > cstInitialFrame && gameSoundWorld) {
			// adjust the sound time here when skipping cinematics (see cstSkipCinematicRequested and cstCinematicSkipped)
		...
		//#modified-fva; BEGIN
		// break the skipCinematic loop if a target_sessionCommand has forced a changelevel; see the doom3 gpl code
...
void idGameLocal::CalcFov(float base_fov, float &fov_x, float &fov_y, bool cstOldStyle) const {
//#modified-fva; END
	...
	float ideal_ratio_x = 16.0f;
	float ideal_ratio_y = 9.0f;
	if (cstOldStyle) {
		ideal_ratio_x = 4.0f;
		ideal_ratio_y = 3.0f;
	}
...
//#modified-fva; BEGIN
bool idGameLocal::CstSkipCinematic() {
	if (common->IsMultiplayer() || !inCinematic || Cst_Shell_IsGameComplete()) {
		return false;
	}
...
//#modified-fva; BEGIN
bool idGameLocal::Cst_Shell_IsShowingIntro() {
	if (!shellHandler) {
		return false;
	}
	return shellHandler->CstIsShowingIntro();
}

// ===============
bool idGameLocal::Cst_Shell_IsGameComplete() {
	if (!shellHandler) {
		return false;
	}
...
//#modified-fva; BEGIN
void idGameLocal::CstRequestCinematicUpdate(const idMaterial * cinematicMaterial, int time) {
	bool found = false;
	for (int i = 0; i < cstCinematicUpdateList.Num(); ++i) {
		CstCinUpdateData & data = cstCinematicUpdateList[i];
		if (data.material == cinematicMaterial) {
			data.time = time;
			found = true;
			break;
		}
	...
	if (!found) {
		CstCinUpdateData & data = cstCinematicUpdateList.Alloc();
		data.material = cinematicMaterial;
		data.time = time;
	}
}

void idGameLocal::CstGenUpdateCinematicCmds() {
	if (cstCinematicUpdateList.Num() == 0) {
		return;
	}
	for (int i = 0; i < cstCinematicUpdateList.Num(); ++i) {
		CstCinUpdateData & data = cstCinematicUpdateList[i];
		renderSystem->CstGenUpdateCinematicCmd(data.material, data.time);
	}
	cstCinematicUpdateList.Clear();
}

void idGameLocal::CstAdjustCinematicUpdateListTime(int time) {
	for (int i = 0; i < cstCinematicUpdateList.Num(); ++i) {
		CstCinUpdateData & data = cstCinematicUpdateList[i];
		if (data.time < 0) {
			data.time = time;
...
void idGameLocal::CstClearCinematicUpdateList() {
	cstCinematicUpdateList.Clear();
}
//#modified-fva; END
