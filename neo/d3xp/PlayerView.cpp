...
void idPlayerView::SingleView( const renderView_t *view, idMenuHandler_HUD * hudManager ) {
	...
	if ( !pm_thirdPerson.GetBool() && !g_skipViewEffects.GetBool() ) {
		...
		if ( alpha < 1.0f  ) {
			...
			renderSystem->CstSetShaderParms(gameLocal.fast.time, idVec4((player->health <= 0.0f) ? MS2SEC(gameLocal.fast.time) : lastDamageTime, 1.0f, 1.0f, (player->health <= 0.0f) ? 0.0f : alpha));
			//#modified-fva; END
		...
		if ( bfgVision ) {
			renderSystem->SetColor4( 1.0f, 1.0f, 1.0f, 1.0f );
			//#modified-fva; BEGIN
			renderSystem->CstSetShaderParms(gameLocal.fast.time, idVec4(1.0f, 1.0f, 1.0f, 1.0f));
			//#modified-fva; END
...
bool idPlayerView::CstCanControlDamageFeedback() const {
	if (!common->IsMultiplayer() || gameLocal.cstSiAllowDamageFeedbackControlMP) {
		return true;
	}
	return false;
}
