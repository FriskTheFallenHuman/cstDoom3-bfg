...
void idSound::Spawn() {
	...
	lastSoundVol	= 0.0f;
	//#modified-fva; BEGIN
	if (cst_no1kMakeMadDrone.GetBool() && refSound.shader) {
		idStr shaderName = refSound.shader->GetName();
		shaderName.StripPath();
		shaderName.StripFileExtension();
		if (shaderName.Icmp("1k_make_mad_drone") == 0) {
			refSound.parms.volume = DB_SILENCE;
			if (refSound.referenceSound) {
				soundShaderParms_t parms;
				memset(&parms, 0, sizeof(parms));
				parms.volume = refSound.parms.volume;
				refSound.referenceSound->ModifySound(SND_CHANNEL_ANY, &parms);
			}
		}
	}
	//#modified-fva; END
