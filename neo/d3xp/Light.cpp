...
//#modified-fva; BEGIN
static bool CstAllowPrelightModel(const char * lightName) {
	if (!lightName || !lightName[0]) {
		return false;
	}
	const char * mapName = gameLocal.GetMapName();
	if (idStr::IcmpPath(mapName, "maps/game/cpu.map") == 0) {
		if (idStr::Icmp(lightName, "light_2417") == 0) {
			return false;
		}
	} else if (idStr::IcmpPath(mapName, "maps/game/mars_city1.map") == 0) {
		if (idStr::Icmp(lightName, "light_4845") == 0 ||
			idStr::Icmp(lightName, "tim_light_3886") == 0 ||
			idStr::Icmp(lightName, "light_4846") == 0) {
			return false;
		}
	} else if (idStr::IcmpPath(mapName, "maps/game/mars_city2.map") == 0) {
		if (idStr::Icmp(lightName, "light_4845") == 0 ||
			idStr::Icmp(lightName, "tim_light_3886") == 0 ||
			idStr::Icmp(lightName, "light_5345") == 0) {
			return false;
		}
	}
	return true;
...
void idLight::Restore( idRestoreGame *savefile ) {
	...
	if (CstAllowPrelightModel(name.c_str())) {
		renderLight.prelightModel = renderModelManager->CheckModel(va("_prelight_%s", name.c_str()));
	} else {
...
void idLight::Spawn() {
	...
	if ( health ) {
		...
		idRenderModel* cstRenderModelBroken = renderModelManager->CheckModel(brokenModel);
		if (cstRenderModelBroken) {
			if (cstRenderModelBroken->IsDefaultModel() && !cst_showDefaultLightModel.GetBool()) {
				brokenModel = "";
				spawnArgs.SetBool("hideModelOnBreak", true);
			}
		} else {
		//#modified-fva; END
			if ( needBroken ) {
	...
	//#modified-fva; BEGIN
	if (renderEntity.hModel && renderEntity.hModel->IsDefaultModel() && !cst_showDefaultLightModel.GetBool()) {
		SetModel("");
