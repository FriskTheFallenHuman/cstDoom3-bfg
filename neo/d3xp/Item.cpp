...
void idItem::GetAttributes( idDict &attributes ) const {
	...
	for( i = 0; i < spawnArgs.GetNumKeyVals(); i++ ) {
		...
		if ( arg->GetKey().Left( 4 ) == "inv_" ) {
			//#modified-fva; BEGIN
			if (arg->GetKey().Mid(4, 5) == "ammo_") {
				bool useOldAmmo = false;
				if (common->IsMultiplayer()) {
					useOldAmmo = gameLocal.cstSiOldAmmoMP;
				} else {
					idPlayer *player = gameLocal.GetLocalPlayer();
					if (player) {
						switch (player->GetExpansionType()) {
						case GAME_BASE: useOldAmmo = cst_oldAmmoD3.GetBool(); break;
						case GAME_D3XP: useOldAmmo = cst_oldAmmoRoE.GetBool(); break;
						case GAME_D3LE: useOldAmmo = cst_oldAmmoLM.GetBool(); break;
						}
					}
				}
				if (useOldAmmo) {
					idStr altKey = "cst_old_" + arg->GetKey();
					idStr altAmmo = spawnArgs.GetString(altKey, "");
					if (!altAmmo.IsEmpty()) {
						attributes.Set(arg->GetKey().Right(arg->GetKey().Length() - 4), altAmmo.c_str());
						continue;
					}
				}
			}
