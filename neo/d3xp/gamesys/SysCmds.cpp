	...
	if ( give_all || idStr::Icmp( name, "weapons" ) == 0 ) {
		...
		if (!common->IsMultiplayer()) {
			...
			if (expansion == GAME_BASE) {
				if (player->cstAwDoubleBarrelShotgunD3 < 1 && player->cst_weapon_shotgun_double > -1) {
					player->inventory.weapons &= ~(int)BIT(player->cst_weapon_shotgun_double);
				}
				if (player->cstAwGrabberD3 < 1 && player->cst_weapon_grabber > -1) {
					player->inventory.weapons &= ~(int)BIT(player->cst_weapon_grabber);
				}
			} else if (expansion == GAME_D3XP) {
				if (player->cstAwChainsawRoE < 1 && player->weapon_chainsaw > -1) {
					player->inventory.weapons &= ~(int)BIT(player->weapon_chainsaw);
				}
			} else if (expansion == GAME_D3LE) {
				if (player->cstAwChainsawLM < 1 && player->weapon_chainsaw > -1) {
					player->inventory.weapons &= ~(int)BIT(player->weapon_chainsaw);
				}
			}
		}
...
void Cmd_God_f(const idCmdArgs &args) {
	...
	if (player->godmode) {
		...
		idStr msg = idLocalization::GetString(CST_STR_GODMODE_OFF);
		common->CstPrintMsg(msg);
	} else {
		player->godmode = true;
		gameLocal.Printf("godmode ON\n");
		idStr msg = idLocalization::GetString(CST_STR_GODMODE_ON);
		common->CstPrintMsg(msg);
	}
}
//#modified-fva; END
...
void Cmd_Notarget_f(const idCmdArgs &args) {
	...
	if (player->fl.notarget) {
		...
		idStr msg = idLocalization::GetString(CST_STR_NOTARGET_OFF);
		common->CstPrintMsg(msg);
	} else {
		player->fl.notarget = true;
		gameLocal.Printf("notarget ON\n");
		idStr msg = idLocalization::GetString(CST_STR_NOTARGET_ON);
		common->CstPrintMsg(msg);
	}
}
//#modified-fva; END
...
void Cmd_Noclip_f(const idCmdArgs &args) {
	...
	if (player->noclip) {
		...
		idStr msg = idLocalization::GetString(CST_STR_NOCLIP_OFF);
		common->CstPrintMsg(msg);
	} else {
		player->noclip = true;
		gameLocal.Printf("noclip ON\n");
		idStr msg = idLocalization::GetString(CST_STR_NOCLIP_ON);
		common->CstPrintMsg(msg);
	}
}
//#modified-fva; END
...
static void Cmd_CstPrintEntityInfo_f(const idCmdArgs &args) {
	...
	if (trace.fraction < 1.0f) {
		...
		if (ent) {
			...
			idStr rotA; rotA.Format("%.9g %.9g %.9g", mat[0].x, mat[0].y, mat[0].z);
			idStr rotB; rotB.Format("%.9g %.9g %.9g", mat[1].x, mat[1].y, mat[1].z);
			idStr rotC; rotC.Format("%.9g %.9g %.9g", mat[2].x, mat[2].y, mat[2].z);
			idStr msgE; msgE.Format(" rotation:\n  %s\n  %s\n  %s\n", rotA.c_str(), rotB.c_str(), rotC.c_str());
			...
			common->CstPrintMsg(msgA);
			common->CstPrintMsg(msgC);
			common->CstPrintMsg(msgD);
		}
	}
}

// ========
CONSOLE_COMMAND_SHIP(cstExportRagdolls, "export all ragdolls matching the specified classname", idCmdSystem::ArgCompletion_Decl<DECL_ENTITYDEF>) {
	if (args.Argc() != 3) {
		common->Printf("Usage: cstExportRagdolls <classname> <outfile>\n");
		return;
	}
	...
	idStr outFilePath = cst_exportPath.GetString();
...
static void Cmd_CstDemigod_f(const idCmdArgs &args) {
	...
	if (player->cstDemigod) {
		...
		gameLocal.Printf("demigod OFF\n");
		idStr msg = idLocalization::GetString(CST_STR_DEMIGOD_OFF);
		common->CstPrintMsg(msg);
	} else {
		player->cstDemigod = true;
		gameLocal.Printf("demigod ON\n");
		idStr msg = idLocalization::GetString(CST_STR_DEMIGOD_ON);
		common->CstPrintMsg(msg);
	}
}
//#modified-fva; END
