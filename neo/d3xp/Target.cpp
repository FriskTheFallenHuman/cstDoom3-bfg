...
void idTarget_EndLevel::Event_Activate( idEntity *activator ) {
	...
	if ( !isTutorialMap && spawnArgs.GetBool( "endOfGame" ) ) {
		...
		if ( player != NULL ) {
			...
			*/
			{
				// notify about the highest difficulty achievement only, but also unlock the others as appropriate
				bool cstNotify = true;
				switch (expansion) {
				case GAME_D3XP:
					// The fall-through is done here on purpose so compleating the game on one difficulty will unlock all the easier difficulties
					switch (g_skill.GetInteger()) {
					case 3: player->GetAchievementManager().EventCompletesAchievement(ACHIEVEMENT_ROE_COMPLETED_DIFFICULTY_3, cstNotify); cstNotify = false;
					case 2: player->GetAchievementManager().EventCompletesAchievement(ACHIEVEMENT_ROE_COMPLETED_DIFFICULTY_2, cstNotify); cstNotify = false;
					case 1: player->GetAchievementManager().EventCompletesAchievement(ACHIEVEMENT_ROE_COMPLETED_DIFFICULTY_1, cstNotify); cstNotify = false;
					case 0: player->GetAchievementManager().EventCompletesAchievement(ACHIEVEMENT_ROE_COMPLETED_DIFFICULTY_0, cstNotify); cstNotify = false;
					}
					break;
				case GAME_D3LE:
					// The fall-through is done here on purpose so compleating the game on one difficulty will unlock all the easier difficulties
					switch (g_skill.GetInteger()) {
					case 3: player->GetAchievementManager().EventCompletesAchievement(ACHIEVEMENT_LE_COMPLETED_DIFFICULTY_3, cstNotify); cstNotify = false;
					case 2: player->GetAchievementManager().EventCompletesAchievement(ACHIEVEMENT_LE_COMPLETED_DIFFICULTY_2, cstNotify); cstNotify = false;
					case 1: player->GetAchievementManager().EventCompletesAchievement(ACHIEVEMENT_LE_COMPLETED_DIFFICULTY_1, cstNotify); cstNotify = false;
					case 0: player->GetAchievementManager().EventCompletesAchievement(ACHIEVEMENT_LE_COMPLETED_DIFFICULTY_0, cstNotify); cstNotify = false;
					}
					break;
				case GAME_BASE:
					// The fall-through is done here on purpose so compleating the game on one difficulty will unlock all the easier difficulties
					switch (g_skill.GetInteger()) {
					case 3: player->GetAchievementManager().EventCompletesAchievement(ACHIEVEMENT_COMPLETED_DIFFICULTY_3, cstNotify); cstNotify = false;
					case 2: player->GetAchievementManager().EventCompletesAchievement(ACHIEVEMENT_COMPLETED_DIFFICULTY_2, cstNotify); cstNotify = false;
					case 1: player->GetAchievementManager().EventCompletesAchievement(ACHIEVEMENT_COMPLETED_DIFFICULTY_1, cstNotify); cstNotify = false;
					case 0: player->GetAchievementManager().EventCompletesAchievement(ACHIEVEMENT_COMPLETED_DIFFICULTY_0, cstNotify); cstNotify = false;
					}
					break;
				}
			}
