...
{
	//#modified-fva; BEGIN
	if (bink_trees_initialized) {
		return;
	}
	bink_trees_initialized = true;
	//#modified-fva; END
...
int BinkDecoder::DecodePlane(BinkCommon::BitReader &bits, int plane_idx, int is_chroma)
{
	...
	*/
	// based on ffmpeg 6.1.1
	int width = frameWidth >> is_chroma;
	int height = frameHeight >> is_chroma;
	//#modified-fva; END
	...
	ref_end = ref_start + (bw - 1 + stride * (bh - 1)) * 8;
	//#modified-fva; END
