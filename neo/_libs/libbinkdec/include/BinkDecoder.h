...
struct Plane
{
	...
	*/
	bool Init(uint32_t width, uint32_t height) {
		// ceil to a multiple of 16 bytes
		const uint32_t widthAdj = (width + 15) & (~15);
		const uint32_t heightAdj = (height + 15) & (~15);

		current = new uint8_t[widthAdj * heightAdj];
		last = new uint8_t[widthAdj * heightAdj];

		widthAllocated = widthAdj;
		heightAllocated = heightAdj;
		widthDisplay = width;
		heightDisplay = height;
		pitch = widthAllocated;
		return true;
	}
	//#modified-fva; END
...
class BinkDecoder
{
	public:
		...
		bool Init(const uint8_t * fileData, int32_t fileDataSize);
		//#modified-fva; END
		...
		enum NextFrameOpt {
			NF_OPT_VIDEO_ONLY,
			NF_OPT_AUDIO_ONLY,
			NF_OPT_AUDIO_AND_VIDEO
		};
		void GetNextFrame(ImagePlane * yuv, NextFrameOpt opt);
		//#modified-fva; END
