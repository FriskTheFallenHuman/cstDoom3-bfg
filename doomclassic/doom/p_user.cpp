...
void P_PlayerThink (player_t* player)
{
	...
	else if ( cmd->buttons & BT_CHANGE )
	{
		...
		if (cmd->cstSsNextPrevWeapon == 1 || cmd->cstSsNextPrevWeapon == 2) {
		//#modified-fva; END
			newweapon = player->readyweapon;
			...
			for ( k = 0; k < NUMWEAPONS; ++k)
			{
				...
				newweapon = (weapontype_t)((cmd->cstSsNextPrevWeapon - 1) ? (newweapon + 1) : (newweapon - 1));
				//#modified-fva; END
			...
			*/
			...
			if (::g->gamemode == commercial) {
				// doom 2
				if (cmd->cstSsNextPrevWeapon == 4) {
					// wp_supershotgun requested
				...
				// doom 1
				if (cmd->cstSsNextPrevWeapon == 4) {
					// wp_supershotgun requested; not available in doom 1
					newweapon = wp_shotgun;
				}
			}
			//#modified-fva; END
	...
	if ((!::g->st_statusbaron || cst_dcScreenInvul.GetInteger() == 0) && player->powers[pw_invulnerability])
	//#modified-fva; END
	{
