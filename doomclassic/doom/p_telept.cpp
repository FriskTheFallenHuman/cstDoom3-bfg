  ...
  mobj_t*	thing )
{
	...
	if (::g->sectors[ i ].tag == tag )
	{
		...
		S_StartSound (fog, sfx_telept);
		*/
		extern idCVar cst_dcNoTeleportFog;
		if (thing->player && thing->player == &::g->players[::g->consoleplayer] && cst_dcNoTeleportFog.GetBool()) {
			S_StartSound(thing->player->mo, sfx_telept);
		} else {
			an = m->angle >> ANGLETOFINESHIFT;
