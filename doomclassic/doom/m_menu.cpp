...
//#modified-fva; BEGIN
extern idCVar cst_dcOldNightmareSP;
//#modified-fva; END
...
void M_ChooseSkill(int choice)
{
	...
	*/
	//#modified-fva; BEGIN
	if (choice == (int)sk_nightmare && cst_dcOldNightmareSP.GetBool()) {
		choice = (int)sk_cst_old_nightmare;
	}
	//#modified-fva; END
	if ( ::g->gamemode != commercial ) {
...
void M_DrawOptions(void)
{
	...
	int fullscreenOnOff = cst_vidFullscreen.GetInteger() == 1 ? 1 : 0;
	//#modified-fva; END
...
void M_FullScreen( int choice ) {
	...
	cst_vidFullscreen.SetInteger(cst_vidFullscreen.GetInteger() == 1 ? 0 : 1);
	//#modified-fva; END
			...
			::g->currentMenu->menuitems[::g->itemOn].status)
		{
			...
			else
			{
				...
				*/
				...
				if (::g->currentMenu == &::g->QuitDef && (::g->itemOn == g_accept || ::g->itemOn == g_change)) {
					cstPlaySound = false;
				}
				::g->currentMenu->menuitems[::g->itemOn].routine(::g->itemOn);
				if (cstPlaySound) {
