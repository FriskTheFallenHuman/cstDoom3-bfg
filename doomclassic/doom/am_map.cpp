...
//#modified-fva; BEGIN
extern idCVar cst_amZoomSpeed;
extern idCVar cst_amPanSpeed;
extern idCVar cst_amFollowOnStart;
//#modified-fva; END
...
void AM_initVariables(void)
{
	...
	//#modified-fva; BEGIN
	::g->cstAmZoomStep = 0.0f;
	if (cst_amFollowOnStart.GetBool()) {
		::g->followplayer = 1;
	}
	//#modified-fva; END
...
( event_t*	ev )
{
	...
	if (!::g->automapactive)
	{
		...
		if (ev->type == ev_cst_automap_keydown && ev->data1 == AM_STARTKEY)
		//#modified-fva; END
		{
			AM_Start ();
			::g->viewactive = false;
	...
	else if (ev->type == ev_cst_automap_keydown)
	//#modified-fva; END
	{
		...
		*/
		case AM_PANRIGHTKEY: { // pan right
			if (!::g->followplayer && !::g->menuactive) ::g->m_paninc.x = (int)((FTOM(F_PANINC)) * cst_amPanSpeed.GetFloat());
			else rc = false;
			break;
		}
		case AM_PANLEFTKEY: { // pan left
		...
		case AM_PANUPKEY: { // pan up
			if (!::g->followplayer && !::g->menuactive) ::g->m_paninc.y = (int)((FTOM(F_PANINC)) * cst_amPanSpeed.GetFloat());
			else rc = false;
			break;
		}
		case AM_PANDOWNKEY: { // pan down
		...
		case AM_ZOOMOUTKEY: { // zoom out
			float speed = cst_amZoomSpeed.GetFloat();
			if (speed == 0.0f) {
				speed = 1.0f;
			}
			::g->mtof_zoommul = (int)((FRACUNIT) / speed);
			::g->ftom_zoommul = (int)((FRACUNIT) * speed);
			break;
		}
		case AM_ZOOMINKEY: { // zoom in
			float speed = cst_amZoomSpeed.GetFloat();
	...
	else if (ev->type == ev_cst_automap_keyup)
	//#modified-fva; END
	{
		rc = false;
		switch (ev->data1)
		...
		*/
		case AM_PANRIGHTKEY:
			if (!::g->followplayer && ::g->m_paninc.x > 0) ::g->m_paninc.x = 0;
			break;
		case AM_PANLEFTKEY:
			if (!::g->followplayer && ::g->m_paninc.x < 0) ::g->m_paninc.x = 0;
			break;
		case AM_PANUPKEY:
			if (!::g->followplayer && ::g->m_paninc.y > 0) ::g->m_paninc.y = 0;
			break;
		case AM_PANDOWNKEY:
			if (!::g->followplayer && ::g->m_paninc.y < 0) ::g->m_paninc.y = 0;
			break;
...
void AM_changeWindowScale(void)
{
	...
	::g->scale_ftom = FixedDiv(FRACUNIT, ::g->scale_mtof);
	*/
	if (::g->cstAmZoomStep > 0.0f) {
		::g->scale_mtof = FixedMul(::g->scale_mtof, (int)((FRACUNIT) * ::g->cstAmZoomStep));
	} else if (::g->cstAmZoomStep < 0.0f) {
		::g->scale_mtof = FixedMul(::g->scale_mtof, (int)((FRACUNIT) / (-::g->cstAmZoomStep)));
	} else {
		::g->scale_mtof = FixedMul(::g->scale_mtof, ::g->mtof_zoommul);
	}
...
void AM_Ticker (void)
{
	...
	if (::g->ftom_zoommul != FRACUNIT || ::g->cstAmZoomStep != 0.0f)
	//#modified-fva; END
