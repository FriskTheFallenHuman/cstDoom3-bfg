...
void G_PerformImpulse( const int impulse, ticcmd_t* cmd ) {
	...
	*/
	...
	case IMPULSE_1: {
		cmd->buttons |= BT_CHANGE;
		cmd->buttons |= wp_fist << BT_WEAPONSHIFT;
		break;
	}
	case IMPULSE_2: {
		cmd->buttons |= BT_CHANGE;
		cmd->buttons |= wp_pistol << BT_WEAPONSHIFT;
		break;
	}
	case IMPULSE_3: {
		cmd->buttons |= BT_CHANGE;
	...
	case IMPULSE_4: {
		// wp_supershotgun
		cmd->buttons |= BT_CHANGE;
		cmd->cstSsNextPrevWeapon = 4; // hack: there is no space in cmd->buttons for wp_supershotgun
		break;
	}
	case IMPULSE_6: {
		cmd->buttons |= BT_CHANGE;
		cmd->buttons |= wp_chaingun << BT_WEAPONSHIFT;
		break;
	}
	case IMPULSE_8: {
		cmd->buttons |= BT_CHANGE;
		cmd->buttons |= wp_plasma << BT_WEAPONSHIFT;
		break;
	}
	case IMPULSE_9: {
		cmd->buttons |= BT_CHANGE;
		cmd->buttons |= wp_missile << BT_WEAPONSHIFT;
		break;
	}
	case IMPULSE_10: {
		cmd->buttons |= BT_CHANGE;
		cmd->buttons |= wp_bfg << BT_WEAPONSHIFT;
		break;
	}
	case IMPULSE_11: {
		cmd->buttons |= BT_CHANGE;
		cmd->buttons |= wp_chainsaw << BT_WEAPONSHIFT;
		break;
	}
	case IMPULSE_14: {
		// next weapon
		cmd->buttons |= BT_CHANGE;
		cmd->cstSsNextPrevWeapon = 2;
		break;
	}
	case IMPULSE_15: {
		// prev weapon
		cmd->buttons |= BT_CHANGE;
		cmd->cstSsNextPrevWeapon = 1;
		break;
	}
	}
	//#modified-fva; END
...
void G_BuildTiccmd (ticcmd_t* cmd, idUserCmdMgr * userCmdMgr, int newTics )
{
	...
	if ( userCmdMgr != NULL ) {
		...
		//if ( curTech5Command.inhibited == false ) {
			...
			*/
			if (::g->cstOldImpulseSequence != curTech5Command.impulseSequence) {
				G_PerformImpulse(curTech5Command.impulse, cmd);
			}
			::g->cstOldImpulseSequence = curTech5Command.impulseSequence;
			//#modified-fva; END
...
void G_DoLoadLevel ()
{
	...
	if (::g->gameskill > sk_nightmare) {
		DoomLib::SetCurrentDifficulty(difficultyNames[sk_nightmare]);
	} else {
		DoomLib::SetCurrentDifficulty(difficultyNames[::g->gameskill]);
	}
	//#modified-fva; END
 ...
 )
{
	...
	if (skill > sk_nightmare)
		skill = sk_nightmare;
	*/
	if (skill > sk_cst_old_nightmare) {
		skill = sk_cst_old_nightmare;
	}
	//#modified-fva; END
	...
	//#modified-fva; BEGIN
	// code below is from the original doom gpl source (with changes)
	if (skill == sk_cst_old_nightmare) {
		if (!::g->cstUsingFastNightmareEnemies) {
			::g->cstUsingFastNightmareEnemies = true;
	...
	} else if (::g->cstUsingFastNightmareEnemies) {
		::g->cstUsingFastNightmareEnemies = false;
		for (i = S_SARG_RUN1; i <= S_SARG_PAIN2; i++) {
			::g->states[i].tics <<= 1;
		}
...
void G_ReadDemoTiccmd (ticcmd_t* cmd)
{
	...
	if (demoversion >= CST_DC_ORIGINAL_VERSION) {
	//#modified-fva; END
